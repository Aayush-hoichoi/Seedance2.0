...
test('shapeUser maps a Clerk backend user to the shared app identity', () => {
    const u = {
        id: 'user_123',
    ...
    assert.deepEqual(shapeUser(u), { userId: 'user_123', email: 'a@hoichoi.tv', name: 'Asha Roy', role: 'admin' });
});
...
test('shapeUser falls back to first email and null role, and returns null for null', () => {
    ...
    assert.deepEqual(shapeUser(u), { userId: 'user_9', email: 'x@y.z', name: 'x', role: null });
