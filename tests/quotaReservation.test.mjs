...
test('an exhausted personal hard limit blocks even while the shared project budget has room', async () => {
...
test('a member with only a personal budget is still bound by it', async () => {
    ...
    const row = await reserve(sql, 5);
    assert.equal(row, null, 'the personal hard limit is exhausted');
});

test('workspace-wide quotas bind independently of personal budgets', async () => {
    const sql = await reservationDb();
