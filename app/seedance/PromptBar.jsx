...
// Sequential-workflow link: pick a finished Green Screen → Mannequin result
// straight into Mannequin mode's video slot — no download/re-upload round-trip.
function MannequinImport({ sources, onImport }) {
    ...
    return (
        <div className="relative shrink-0">
            ...
            >
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><circle cx="12" cy="5" r="2.5" /><path d="M12 8v6M8 11h8M12 14l-3 6M12 14l3 6" /></svg>
                <span className="text-[11px] font-semibold">Import mannequin</span>
            </button>
            {open && (
                <Popover>
                        ...
                        {sources.map((j) => (
                            ...
                            >
                                ...
                                <div className="min-w-0">
                                    <div className="text-xs text-white/80">Mannequin</div>
                                    <div className="text-[10px] text-white/40">{j.createdAt ? new Date(j.createdAt).toLocaleString() : ''}</div>
                                </div>
                            </button>
                        ))}
                    </div>
