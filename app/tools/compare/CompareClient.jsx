...
// The caller's own generated videos, newest first (same source as the studio
// history rail). Text rows keep this light — no 200 video previews at once.
function StudioPicker({ onClose, onPick }) {
    ...
    useEffect(() => {
        ...
        fetch('/api/gallery?mine=1')
            .then((r) => (r.ok ? r.json() : Promise.reject(new Error(`Could not load your videos (${r.status}).`))))
            .then((d) => {
                if (!alive) return;
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm" onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
            <section role="dialog" aria-modal="true" className="flex max-h-[80vh] w-full max-w-2xl flex-col rounded-2xl border border-line bg-paper-1 p-4 shadow-2xl">
                ...
                <input autoFocus value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search your prompts…"
                    className="mb-3 rounded-md border border-line bg-paper-3 px-3 py-2 text-sm text-ink outline-none focus-visible:ring-2 focus-visible:ring-accent" />
                <div className="min-h-0 flex-1 overflow-y-auto">
                    {error ? <p className="p-3 text-xs text-danger">{error}</p>
                        : items == null ? <p className="p-3 text-xs text-ink-3">Loading your videos…</p>
                            : !shown.length ? <p className="p-3 text-xs text-ink-3">{query ? 'No videos match that search.' : 'No finished videos in your studio yet.'}</p>
                                : (
