...
export async function inlineImageBlocksForResults(results, {
    budgetBytes = INLINE_IMAGE_BUDGET,
    maxImages = Number.POSITIVE_INFINITY,
} = {}) {
    const blocks = [];
    let budget = budgetBytes;
    const images = results.flatMap((result) => Array.isArray(result?.images) ? result.images : []);
    for (const im of images) {
        if (blocks.length >= maxImages) break;
        if (im?.b64) { // dev fallback: already inline
            const byteLength = Buffer.byteLength(im.b64, 'base64');
            if (byteLength > budget) continue;
            budget -= byteLength;
...
export async function inlineImageBlocks(result, options) {
    return inlineImageBlocksForResults([result], options);
}
