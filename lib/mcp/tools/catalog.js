...
export function registerCatalogTools(server) {
    registerTool(server, {
        ...
        run: async ({ user }) => {
            ...
            // Video models carry resolution capability flags directly on the
            // model entry (lib/seedance/constants.js MODELS: supports1080p,
            // supports4k, supportsReference) — not nested under a caps object.
            // Image models (IMAGE_MODELS) don't have these flags at all; their
            // resolution choices are fully expressed by `resolutions`.
            const shape = (m, category) => ({
                ...
                caps: category === 'video'
                    ? { supports1080p: !!m.supports1080p, supports4k: !!m.supports4k, supportsReference: !!m.supportsReference }
                    : null,
            });
            return {
    ...
    registerTool(server, {
        ...
        run: async ({ user, args }) => {
            ...
            const { id, status, fresh } = await requestAccess(user.userId, user.email, args.modelId, args.note ?? null, args.projectId);
            if (fresh) {
