...
async function inlineImageBlocks(result) {
    const blocks = [];
...
export function registerGenerateTools(server) {
    ...
    registerTool(server, {
        name: 'get_job_status',
        description: 'Status of a generation. Pass generationId (gateway id from create_image / jobId from create_video) OR taskId (ModelArk id from create_video). While status is queued/running, KEEP POLLING: call this tool again every ~8 seconds until it reaches succeeded or failed. Finished images are returned inline (they render in the chat); finished videos return a playable presigned videoUrl to share with the user.',
        schema: {
