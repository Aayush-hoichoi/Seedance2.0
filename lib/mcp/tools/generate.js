...
import { z } from 'zod';
import { registerTool, toolGatewayCtx, ToolError, rawContent } from '../register.js';
...
import { presignKey, imageUrlsFromResult } from '../../seedance/galleryItem.mjs';
import { archiveKeyForTask } from '../../seedance/archiveKey.mjs';
import { inlineImageBlocks } from '../inlineImages.mjs';
...
export function registerGenerateTools(server) {
    ...
    registerTool(server, {
        name: 'get_job_status',
        description: 'Status of a generation. Pass generationId (gateway id from create_image / jobId from create_video) OR taskId (ModelArk id from create_video). While status is queued/running, KEEP POLLING: call this tool again every ~8 seconds until it reaches succeeded or failed. Finished images come back INLINE in this tool result and render in the chat — whenever the user asks to see/get/show the image, call THIS tool with the generationId; never try to download imageUrls with other tools. Finished videos return a playable presigned videoUrl link to give the user.',
        schema: {
        ...
        run: async ({ user, args }) => {
            if (args.generationId) {
                ...
                const payload = {
                    ...
                    imageUrls: isVideo ? null : imageUrlsFromResult(job.result),
                };
                if (job.status === 'queued' || job.status === 'running') {
                    payload.next = 'Still generating — call get_job_status again in ~8 seconds; keep polling until status is succeeded or failed.';
                }
                if (!isVideo && job.status === 'succeeded') {
                    return rawContent([
                        { type: 'text', text: JSON.stringify(payload, null, 2) },
                        ...(await inlineImageBlocks(job.result)),
                    ]);
                }
                return payload;
            }
            ...
            return {
                ...
                ...(/succeed|fail|cancel/i.test(status) ? {} : {
                    next: 'Still generating — call get_job_status again in ~8 seconds; keep polling until the task succeeds or fails.',
                }),
                raw: data,
            };
        },
