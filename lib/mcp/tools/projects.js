...
import { styleSummary } from '../../gateway/projectStyle.mjs';

export function registerProjectTools(server) {
    registerTool(server, {
        name: 'list_projects',
        description: "Projects you can act on, with member count, spend, and each project's saved style looks. A project whose style is non-null applies that look to every generation automatically; pass its key as styleLook to pick a different one, or 'none' to skip it.",
        run: async ({ user }) => {
            ...
            const rows = canManageProjects
                ...
                : await sql`SELECT p.*, m2.role AS my_role,
                   ...
                   WHERE p.archived_at IS NULL ORDER BY p.created_at`;
            // Summary only, never the brief prose: an agent needs the look keys
            // to pass styleLook to create_video/create_image, and the briefs
            // would be several KB per project of context it cannot act on.
            const items = rows.map(({ style, ...project }) => ({ ...project, style: styleSummary(style) }));
            return { items, canManageProjects };
        },
    });
