...
import { overCommitsProject, usageForQuotas, writeAudit } from '../../gateway/db.js';
...
export function registerAdminTools(server) {
    ...
    registerTool(server, {
        ...
        run: async ({ user, args }) => {
            ...
            // The override is what the gateway enforces; the status row above
            // only records the decision. Fail loudly rather than reporting a
            // grant the user cannot actually use.
            try {
                ...
                throw new ToolError('SYNC_FAILED', `Decision saved, but applying it failed: ${error.message}. Access is unchanged.`);
            }
    ...
    registerTool(server, {
        ...
        run: async ({ user, args }) => {
            const { sql } = await toolGatewayCtx(user, { permission: 'quota.manage' });
            if (args.userId && !args.modelId) {
                throw new ToolError('BAD_REQUEST', 'Per-user budgets must name a specific model — all-models budgets for a user are disabled.');
            }
            if (args.modelId) {
            ...
            const overCommit = await overCommitsProject(sql, {
                projectId: args.projectId ?? null, userId: args.userId ?? null,
                type: args.type, window: args.window, nextLimit: args.hardLimit,
            });
            if (overCommit) {
                throw new ToolError('BUDGET_EXCEEDS_PROJECT_CAP',
                    `This exceeds the project's overall budget of $${overCommit.overallCap.toFixed(2)}. Members are already allotted $${overCommit.allocated.toFixed(2)}, leaving $${overCommit.available.toFixed(2)}.`);
            }
