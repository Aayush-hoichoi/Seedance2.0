...
import { listCreators, listUserGenerations, listLikedGenerations, getTaskOwner, recordGenerationEvent } from '../../access/db.js';
...
export function registerHistoryTools(server) {
    ...
    registerTool(server, {
        name: 'get_generation',
        description: 'One generation by gateway id: full job row. Finished images come back INLINE in the result (they render in the chat); finished videos carry a presigned archiveUrl link.',
        ...
        run: async ({ user, args }) => {
            ...
            const payload = { ...redact(job, own, seePrompts), archiveUrl, imageUrls };
    ...
    registerTool(server, {
        ...
        run: async ({ user, args }) => {
            ...
            if (args.liked) {
                return { items: (await listLikedGenerations(user.userId)).map((r) => ({
                    ...toItem(r),
    ...
    registerTool(server, {
        ...
        run: async ({ user, args }) => {
            ...
            // Per-user event for the dataset signal (best-effort).
            await recordGenerationEvent(sql, { taskId: args.taskId, userId: user?.userId ?? null, eventType: args.value ? 'like' : 'unlike' });
