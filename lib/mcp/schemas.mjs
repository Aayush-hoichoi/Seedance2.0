...
// Which of the project's saved style looks to apply to this generation.
// Omitted = the project's default look; 'none' = skip the project style for
// this one generation. A project with no saved style ignores it either way.
const styleLookShape = z.string().max(64).optional()
    .describe("Project style look to apply (e.g. 'chase-window'), or 'none' to skip the project style. Omit for the project default. See list_projects for a project's available looks.");

// A workspace workflow (a named reusable style, project-independent). When
// given, its style overrides the project's own for this generation.
const workflowIdShape = z.number().int().positive().optional()
    .describe('Workspace workflow id whose style governs this generation (overrides the project style).');
...
export const createVideoShape = {
    ...
    styleLook: styleLookShape,
    workflowId: workflowIdShape,
};
...
export const TOOL_PERMISSIONS = {
    ...
    wait_for_generations: null, display_generation: null, display_generations: null,
