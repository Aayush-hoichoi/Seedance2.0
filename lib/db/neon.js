...
// persist the prompt pair (user's raw prompt + the enhancer-generated brief) per
...
import { GATEWAY_DDL, SCHEMA_VERSION, shouldRunGatewaySchema } from './schema.mjs';
import { seedGateway } from './seeds.mjs';

// Skip the ~30-statement DDL+seed chain when this database already carries
// this schema version or a newer one. Older deployments must never replay
// their DDL against a newer database and accidentally attempt a downgrade.
async function ensureGatewaySchema(sql) {
    try {
        const [row] = await sql`SELECT value FROM gateway_state WHERE key = 'schema.version'`;
        if (!shouldRunGatewaySchema(row?.value?.v)) return;
    } catch (e) {
...
export async function getDb() {
    ...
    if (!tableReady) {
        tableReady = sql`CREATE TABLE IF NOT EXISTS seedance_prompts (
            ...
            // project_id: a request is scoped to the project the user asked from,
            // so approval grants the per-user override on THAT project (not
            // auto-Default) — the gateway enforces access per project.
            .then(() => sql`ALTER TABLE model_access_requests ADD COLUMN IF NOT EXISTS project_id integer`)
            // Requests are per (user, model, project): the old (user_id, model_id)
            // unique blocked requesting the same model for a second project. Swap
            // it for a per-project unique index (idempotent, retag-safe).
            .then(() => sql`ALTER TABLE model_access_requests DROP CONSTRAINT IF EXISTS model_access_requests_user_id_model_id_key`)
            .then(() => sql`CREATE UNIQUE INDEX IF NOT EXISTS model_access_requests_user_model_project_uidx ON model_access_requests (user_id, model_id, project_id)`)
            // max_resolution: the quality tier the user asked for; overwritten
            // by the admin's choice on approve (they may grant lower than asked).
            // NULL = unlimited. Mirrored onto user_model_overrides for enforcement.
            .then(() => sql`ALTER TABLE model_access_requests ADD COLUMN IF NOT EXISTS max_resolution text`)
            // pending_max_resolution: a tier-UPGRADE ask on a live grant. The row
            // stays approved (nothing re-locks) while this holds the wanted tier;
            // approve moves the admin's tier into max_resolution + the override
            // and clears it, deny just clears it. Invariant: only ever non-NULL
            // on an approved row.
            .then(() => sql`ALTER TABLE model_access_requests ADD COLUMN IF NOT EXISTS pending_max_resolution text`)
