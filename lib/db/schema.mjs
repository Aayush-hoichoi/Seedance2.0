...
export const GATEWAY_DDL = [
    ...
    // send/decide-via-signed-link design as teams_budget_cards, applied to the
    // second request type. request_id is an INTEGER here (model_access_requests.id
    // is `serial`), unlike teams_budget_cards.request_id (audit_log's target_id,
    // text) — the two tables are never queried against each other, and a
    // signed link's `kind` field (lib/teams/magicLink.mjs) is what stops a
    // budget-request token from ever being replayed against this table's rows
    // even where the raw ids collide.
