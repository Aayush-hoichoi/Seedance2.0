...
// stored version is current or newer, keeping cold starts cheap and preventing
// an older deployment from replaying downgrade-shaped DDL.
export const SCHEMA_VERSION = 27;

// Whether this build should apply its schema chain to the connected database.
// Kept pure so version-skew behavior can be regression-tested without touching
// a real database.
export function shouldRunGatewaySchema(databaseVersion, codeVersion = SCHEMA_VERSION) {
    const current = Number(databaseVersion);
    return !Number.isFinite(current) || current < codeVersion;
}
...
export const GATEWAY_DDL = [
    ...
    // EXR MediaKit queue ----------------------------------------------------
    // Separate from the model generation queue because EXR enhancement is a
    // long-running provider task with its own worker and retry policy.
    `CREATE TABLE IF NOT EXISTS exr_jobs (
        id serial PRIMARY KEY,
        project_id integer,
        user_id text NOT NULL,
    ...
    `CREATE INDEX IF NOT EXISTS exr_jobs_runnable_idx
        ON exr_jobs (status, run_after, created_at)`,
    `CREATE INDEX IF NOT EXISTS exr_jobs_user_idx
        ON exr_jobs (user_id, created_at DESC)`,
    ...
    `CREATE OR REPLACE VIEW gallery_generations AS
        SELECT
            ...
            -- v17→v18: project scope + which project style fired. Appended last
            -- so CREATE OR REPLACE stays valid. project_id was never exposed
            -- here, which made per-project evaluation impossible downstream.
            j.project_id                                                         AS project_id,
            j.request_body->'options'->>'style_look'                             AS style_look,
            (j.request_body->'options'->>'style_version')::int                   AS style_version,
            -- v19→v20: the FULL stored image array. image_key (index 0) is all
            -- the reads ever surfaced, so a 4-image job showed one image in
            -- history and the other three were invisible. Appended last so
            -- CREATE OR REPLACE stays valid.
            j.result->'images'                                                   AS images,
            -- v21→v22: which workspace workflow styled this generation, so the
            -- self-refresh loop and per-workflow scorecards can attribute
            -- likes. Appended last so CREATE OR REPLACE stays valid.
            (j.request_body->'options'->>'style_workflow')::int                  AS style_workflow
        FROM jobs j
        ...
        UNION ALL
        SELECT
            ue.task_id, ue.user_id, ue.user_email, ue.model_id, ue.resolution,
            ue.duration, ue.ratio, ue.mode,
            CASE WHEN ue.status = 'created' THEN 'running' ELSE ue.status END,
            ue.has_video_input, ue.created_at, 'video',
            NULL, NULL, NULL, ue.project_id, NULL, NULL::int, NULL::jsonb, NULL::int
        FROM usage_events ue
        WHERE ue.task_id IS NOT NULL
            AND NOT EXISTS (SELECT 1 FROM jobs j2 WHERE j2.provider_task_id = ue.task_id)`,

    // v19→v20 (cont.): the UNION arm above restores generations the gateway
    // never recorded. createVideoTask "fails open" when resolveGateway errors:
    // the video still reaches the provider and logUsage writes usage_events —
    // but no jobs row, so the generation was permanently invisible in
    // history/gallery. usage-only rows (task ids absent from jobs) are read
    // back in, video-shaped, with no image/style columns. 'created' maps to
    // 'running' so an unfinalized row renders as in-flight, not as an error.
    // The index makes the per-row NOT EXISTS probe an index hit.
    `CREATE INDEX IF NOT EXISTS jobs_provider_task ON jobs (provider_task_id)`,
    ...
    `CREATE OR REPLACE VIEW dataset_samples AS
        SELECT
            ...
            -- v17→v18: the evaluation substrate for per-project style memory.
            -- prompt (above) keeps its meaning: what the user/enhancer wrote, as
            -- the browser recorded it. sent_prompt is what the provider
            -- actually received, which is the only text the style layer can be
            -- judged on. They differ exactly when a project style fired.
            -- Appended last so CREATE OR REPLACE stays valid.
            g.image_prompt                                                       AS sent_prompt,
            g.project_id,
            g.style_look,
            g.style_version,
            -- v21→v22: workflow attribution for the refresh loop. Appended
    ...
    `CREATE OR REPLACE VIEW generation_ledger AS
        ...
        SELECT
            ...
            -- Reconciling drift, not a new feature: the live view already had
            -- this column, appended by a deploy whose DDL never reached this
            -- file. It stayed invisible because getDb() skips the chain while
            -- schema.version matches, so the chain was never replayed against
            -- it. The next version bump WOULD replay it, and CREATE OR REPLACE
            -- VIEW refuses to drop a column — so the bump would have failed
            -- every cold start until this line existed. Keep it last.
            j.finished_at
        FROM jobs j
        LEFT JOIN users u ON u.id = j.user_id
    ...
    // v17→v18: per-project style memory, applied by lib/gateway/projectStyle.mjs
    // to every generation the gateway governs. Shape:
    //   { enabled, version, defaultLook, looks: { <key>: { name, brief, negatives } },
    //     characters: { <name>: <description> } }
    // NULL means "no style" — that is the per-project opt-in, so there is no
    // flag table. gatewayContextFor already SELECTs the whole projects row, so
    // reading this costs no extra query on the hot path.
    `ALTER TABLE projects ADD COLUMN IF NOT EXISTS style jsonb`,

    // v20→v21: workspace-level workflows. A workflow is a named, reusable style
    // (same jsonb shape as projects.style, validated by the same styleError) a
    // user can attach in the studio regardless of which project they are in.
    // An attached workflow overrides the project's own style for that request.
    ...
    // The user's attached workflow — server-side so it follows them across
    // browsers/devices and so MCP/API generations inherit it without the
    // caller passing an id. NULL = nothing attached.
    `ALTER TABLE users ADD COLUMN IF NOT EXISTS workflow_id integer`,

    // v23→v24: workflows are gated ONCE per user, not per workflow. A user
    // requests workflow access, an admin decides in the console Requests hub,
    // and approval unlocks every workflow (platform admins bypass). One row
    // per user; a denied request can be re-asked by flipping back to pending.
    ...
    // v24→v25: Teams cards for workflow-access requests — same send-and-update
    // design as teams_access_cards, keyed by the requesting user (workflow
    // access is one request per user, so user_id IS the request id).
    // v25→v26: user-created workflows. NULL created_by = official (visible to
    // everyone with workflow access); a user's own customs are visible only
    // to them. Same style schema, same gating, same nightly self-refresh.
    `ALTER TABLE workflows ADD COLUMN IF NOT EXISTS created_by text`,
    // v26→v27: sharing a custom takes BOTH sides wanting it — the owner asks
    // ('pending'), an admin approves ('public'). Officials ignore this column.
    `ALTER TABLE workflows ADD COLUMN IF NOT EXISTS visibility text NOT NULL DEFAULT 'private'`,

    `CREATE TABLE IF NOT EXISTS teams_workflow_cards (
        user_id         text NOT NULL,
        aad_object_id   text NOT NULL,
        conversation_id text NOT NULL,
        activity_id     text NOT NULL,
        state           text NOT NULL DEFAULT 'pending',
        updated_at      timestamptz NOT NULL DEFAULT now(),
        PRIMARY KEY (user_id, aad_object_id)
    )`,
];
