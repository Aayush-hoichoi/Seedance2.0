...
import { TOOL_CATALOG } from '../tools/catalog.mjs';
...
const PERMISSIONS = [
    ...
    // The generation ledger carries every prompt, cost and user email in one
    // place. Kept as its own permission rather than folded into audit.view or
    // prompt.view so it can be widened later (to managers, say) without also
    // handing out the audit trail — and narrowed without touching either.
    ['ledger.view', 'View and export the generation ledger'],
];
...
export function catalog() {
    ...
    return [
        ...
        // bypasses the approval flow entirely.
        {
            ...
            // Capability flags are also used by the access-quality picker. Keep
            // the existing model-version row in sync when this catalog changes.
            // 4k: live-probed 2026-09-22, rejected by ModelArk for this account
            // (see the ledger in lib/seedance/constants.js).
        ...
        {
            // Dedicated "sensitive content" endpoint (BytePlus console), backed by
            // Dreamina-Seedance-2.0 260128 — same key, same API, endpoint-level
            // moderation. Never a default: access is grant-only, like the other
            // gated tiers.
            id: 'seedance-2.0-sensitive', display: 'Sensitive Content', category: 'video', isDefault: false, kind: 'full_sensitive',
            provider: 'byteplus',
            providerModelId: env.NEXT_PUBLIC_SEEDANCE_SENSITIVE_MODEL_ID || 'ep-20260903173403-mctqm',
            caps: { supports1080p: true, supports4k: true },
            route: { mode: 'interactive', status: 'active' },
        },
        {
        ...
        {
            // ChatGPT Image 2.5 — OpenAI's images API called directly with the
            // org's own OPENAI_API_KEY (the same env the prompt enhancer already
            // uses, so a deployed app needs no new secret). Flare is the fast
            // everyday tier; OPENAI_IMAGE_MODEL_ID switches to -sunburst. Sync
            // like Seedream: submit returns the images inline, no task to poll,
            // so the 300s image-class default timeout is plenty.
            id: 'chatgpt-image-2.5', display: 'ChatGPT Image 2.5', category: 'image', isDefault: false, kind: 'chatgpt_image_2_5',
            provider: 'openai',
            providerModelId: env.OPENAI_IMAGE_MODEL_ID || 'gpt-image-2.5-flare',
            caps: {},
            route: { mode: 'interactive', status: 'active' },
        },
        {
...
export async function seedGateway(sql) {
    ...
    await sql`INSERT INTO providers (id, display_name)
        VALUES ('byteplus', 'BytePlus ModelArk'), ('google', 'Google Gemini'), ('kie', 'kie.ai'), ('openai', 'OpenAI')
    ...
    // Tools: catalog rows only (no version/route — they don't run through the
    // gateway), so access requests and budgets can target them.
