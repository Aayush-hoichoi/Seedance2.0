...
// fetch() rejects with an opaque "Failed to fetch" when the dev server is down;
// translate that into something actionable.
function networkError() {
    return new Error('Could not reach the app server — make sure `npm run dev` is running, then try again.');
}
...
async function getJson(path) {
    ...
    const data = await res.json().catch(() => null);
    if (!res.ok) throw new Error(extractError(data, `Poll failed (${res.status})`));
    return data;
}
...
export async function pollTask(id, { onStatus, signal } = {}) {
    for (let attempt = 0; attempt < POLL_MAX_ATTEMPTS; attempt++) {
        ...
        const data = await getJson(`${TASKS_PATH}/${id}`);
