...
import { MODELS, POLL_INTERVAL_MS, POLL_MAX_ATTEMPTS, supportedResolutionsFor } from './constants.js';
...
export function buildPayload({ options, prompt, mediaItems }) {
    ...
    // Keep the browser boundary aligned with the server capability check. This
    // matters for stale settings and direct callers: native 4K is a standard
    // Seedance 2.0 capability, not a generic Seedance-family tier. The gateway
    // remains authoritative, but failing here gives the user a useful error
    // before a task is created or budget is reserved.
    const model = MODELS.find((m) => m.id === options.model);
    const supported = supportedResolutionsFor(options.model);
    if (model && supported && options.resolution
        && !supported.some((tier) => tier.toLowerCase() === String(options.resolution).toLowerCase())) {
        throw new Error(`${model.name} does not support ${options.resolution}. Select a supported resolution or switch to a model with native 4K enabled.`);
    }
    ...
    // Seedance 2.5 extras — the caller sets these only when the model takes
    // them (2.0 does not know the params). omni_reference_task_type moves the
    // task-type constraint check to submit time; output_format defaults to mp4
    // provider-side, so only a non-default value is worth sending.
    if (options.omni_reference_task_type) payload.omni_reference_task_type = options.omni_reference_task_type;
    if (options.output_format && options.output_format !== 'mp4') payload.output_format = options.output_format;
    return payload;
}
...
// Create a generation task. Returns the task id. `mode` (optional) is forwarded
// as a header so the server can log the studio mode with the usage row;
// `projectId` scopes the request to a gateway project for access + budgets.
export async function createTask(payload, mode, projectId, styleLook, workflowId) {
    const headers = {};
    if (mode) headers['x-seedance-mode'] = mode;
    if (projectId) headers['x-seedance-project'] = String(projectId);
    // A header, not a payload field: buildPayload whitelists what ModelArk
    // accepts, and the provider must never see our routing metadata.
    if (styleLook) headers['x-seedance-style-look'] = String(styleLook);
    // The attached workspace workflow — its style governs this generation.
    if (workflowId) headers['x-seedance-workflow'] = String(workflowId);
    const data = await postJson(TASKS_PATH, payload, headers);
    const id = data?.id;
    if (!id) throw new Error('ModelArk did not return a task id.');
    // style: { look, workflowId } when the server composed a style into the
    // prompt, null when it did not — the UI shows this rather than assuming.
    return { id, style: data?.style ?? null };
}
