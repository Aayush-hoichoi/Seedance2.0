...
export const IMAGE_LIMITS = {
    // jpeg/png/webp/bmp/tiff/gif always; heic/heif on Seedance 1.5 Pro, 2.0 & 2.5
    // (2.5 tutorial lists them explicitly, doc revision 2026-08-31).
...
// rejects. When the task subtype is left to the model (taskType auto/edit),
// a clip has to satisfy the editing window to be safe, since Seedance may
// still classify the prompt as an edit while it renders. A DECLARED
// reference task (omni_reference_task_type='reference', doc revision
// 2026-09-04, live-probed 2026-09-08 — see constraints25.mjs) can't be run
// as an edit, so the plain reference window of 2–30s applies instead.
// maxCount 10 per the 2026-09-04 doc revision ("up to 10 reference videos").
const VIDEO_LIMITS_2_5 = { minDurationSec: 4, maxDurationSec: 30, maxTotalDurationSec: 30, maxCount: 10 };
const REFERENCE_CLIP_MIN_SEC = 2; // 2.5 style/motion reference floor (tutorial: "2–30s")

export const VIDEO_LIMITS = {
    formats: ['mp4', 'mov'], // encodings: H.264/AVC, H.265/HEVC; embedded audio AAC/MP3
    // Doc revision of 2026-09-04 raised the reference-video window: 4k input
    // (3840×2160 = 8,294,400 px) is now accepted, size cap 50 → 200 MB. The
    // old 2,086,876-px (~1080p) cap rejected 4k refs the provider now takes.
    maxBytes: 200 * 1024 * 1024,
    minDim: 300,
    maxDim: 6000,
    minTotalPx: 407696, // 614 × 664
    maxTotalPx: 8295044, // 3326 × 2494 (covers 4k UHD)
    minAspect: 0.4,
    maxAspect: 2.5,
    minFps: 24,
    maxFps: 60,
...
// Seedance 2.5 raises the audio allowance too (tutorial, 2026-08-31 revision):
// "up to 10 reference audio clips ... total duration must not exceed 30 s",
// single clip 2–30s. Same opt-in override pattern as video below.
const AUDIO_LIMITS_2_5 = { maxDurationSec: 30, maxTotalDurationSec: 30, maxCount: 10 };
...
export const REFERENCE_IMAGE_MAX = 9; // multimodal reference: 0–9 images (2.0 spec)
const REFERENCE_IMAGE_MAX_2_5 = 30; // 2.5: "1–30 images" (doc revision 2026-08-31)
...
const AUDIO_LIMIT_OVERRIDES = { 'full_2_5': AUDIO_LIMITS_2_5 };

export function videoLimitsFor(modelKind, taskType = null) {
    const override = VIDEO_LIMIT_OVERRIDES[modelKind];
    if (!override) return VIDEO_LIMITS;
    const limits = { ...VIDEO_LIMITS, ...override };
    // Declared reference task → the edit-only 4s floor doesn't apply.
    if (taskType === 'reference') limits.minDurationSec = REFERENCE_CLIP_MIN_SEC;
    return limits;
}

export function audioLimitsFor(modelKind) {
    const override = AUDIO_LIMIT_OVERRIDES[modelKind];
    return override ? { ...AUDIO_LIMITS, ...override } : AUDIO_LIMITS;
}

export function referenceImageMaxFor(modelKind) {
    return modelKind === 'full_2_5' ? REFERENCE_IMAGE_MAX_2_5 : REFERENCE_IMAGE_MAX;
}
...
export function validateVideoMetadata({ width, height, durationSec, fps }, modelKind = null, taskType = null) {
    const L = videoLimitsFor(modelKind, taskType);
    if (width && height) {
...
export function validateAudioMetadata({ durationSec }, modelKind = null) {
    const L = audioLimitsFor(modelKind);
    if (durationSec != null && (durationSec < L.minDurationSec || durationSec > L.maxDurationSec)) {
...
export function validateAggregate(mediaItems, modelKind = null) {
    ...
    const AUDIO = audioLimitsFor(modelKind);
    ...
    if (images.length > referenceImageMaxFor(modelKind)) return `Too many reference images (${images.length}); max is ${referenceImageMaxFor(modelKind)}.`;
    if (videos.length > VIDEO.maxCount) return `Too many reference videos (${videos.length}); max is ${VIDEO.maxCount}.`;
    if (audios.length > AUDIO.maxCount) return `Too many reference audio files (${audios.length}); max is ${AUDIO.maxCount}.`;
    ...
    if (aTotal > AUDIO.maxTotalDurationSec) {
        return `Total reference-audio duration is ${aTotal.toFixed(1)}s — combined must not exceed ${AUDIO.maxTotalDurationSec}s.`;
    }
    return null;
}
