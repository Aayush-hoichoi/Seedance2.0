...
export function sanitizeOptions(raw, { defaults, modelIds, ratios, resolutions, modelSupports1080p, modelSupports4k, modelDurationMax = null }) {
    ...
    // Seedance 2.5 extras. Literal sets, same trade-off as DURATION_FLOOR
    // above: these are API constants, not catalog state.
    const taskType = ['auto', 'reference', 'edit', 'extend'].includes(o.taskType) ? o.taskType : (defaults.taskType ?? 'auto');
    const output_format = ['mp4', 'mov'].includes(o.output_format) ? o.output_format : (defaults.output_format ?? 'mp4');

    return { model, ratio, resolution, duration, generate_audio, watermark, seed, taskType, output_format };
}
