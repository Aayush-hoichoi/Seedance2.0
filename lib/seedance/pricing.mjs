...
const RATES = {
    ...
    // Seedance 2.5 — PROVISIONAL: BytePlus has not published 2.5 rates, so these
    // mirror `full` (2.0). Deliberately not left absent: an unknown kind makes
    // estimateCost() return null, which means NO budget reservation, so the tier
    // would generate outside quota enforcement entirely. Over-estimating only
    // reserves too much and is corrected by costFromTokens() on settle; the real
    // rates must land here before 2.5 is activated.
    full_2_5: { sd: [7.0, 4.3], '1080p': [7.7, 4.7], '4k': [4.0, 2.4] },
    // Sensitive Content endpoint — same backing model as `full` (Dreamina-
    // Seedance-2.0 260128), so identical rates.
    full_sensitive: { sd: [7.0, 4.3], '1080p': [7.7, 4.7], '4k': [4.0, 2.4] },
    fast: { sd: [5.6, 3.3] },
    mini: { sd: [3.5, 2.1] },
...
const EXAMPLE_5S = {
    ...
    full_2_5: { '480p': 0.35, '720p': 0.76, '1080p': 1.87, '4k': 3.89 }, // provisional, see RATES
    full_sensitive: { '480p': 0.35, '720p': 0.76, '1080p': 1.87, '4k': 3.89 }, // same backing model as `full`
...
// Adaptive/unknown duration (-1) must be BUDGETED at the kind's maximum length:
// the provider picks the real length, and pricing "Auto" as 5s let adaptive
// videos settle ~3× over hard budget caps (Sep 2026: quotas 171 and 126 —
// $2.19 estimated, ~$6.18 settled per 15s video). Maxima mirror
// lib/seedance/limits.js; kept inline because this module stays import-free.
const MAX_DURATION_SEC = { full_2_5: 30 };
export function budgetDuration(kind, duration) {
    if (typeof duration === 'number' && duration > 0) return duration;
    return MAX_DURATION_SEC[kind] ?? 15;
}
...
// with the exact token-based cost anyway. Unknown duration prices at the max
// (see budgetDuration): over-reserving self-corrects at settlement, while
// under-reserving is how spend crossed hard caps.
export function estimateCost({ kind, resolution, duration, hasVideoInput = false }) {
    ...
    const dur = budgetDuration(kind, duration);
