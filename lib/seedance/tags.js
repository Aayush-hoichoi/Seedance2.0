...
// typed) into the exact "@Image N" wording Seedance binds references by.
//
// This used to STRIP the @ and emit a bare "Image 1", on the belief that was
// what the API wanted. It is the opposite: every example in the ModelArk docs
// keeps it —
//
//   "referring to the composition of @Video1"
//   "replace the character in @Video 1 with the character in @Image 1"
//
// and the prompt rules say outright: "Use @Image 1, @Video 1, and @Audio 1 to
// refer to reference assets." Without the @, the model receives the assets but
// nothing tying them to the words, so it free-generates from the prose — which
// is exactly what shipped: four references attached, none of them honoured.
export function normalizePromptForApi(prompt) {
    return prompt.replace(TOKEN_RE, (_, kind, n) => `@${KIND_CAP[kind.toLowerCase()]} ${n}`);
}

// Inverse of normalizePromptForApi: rewrite the API wording back into "@ImageN"
// mention tokens so the cyan chips repaint. Used by Reuse.
//
// Still handles the BARE "Image N" form as well as "@Image N": prompts stored
// before the @ was preserved carry the bare wording, and Reuse has to repaint
// those too. Only references that actually exist (N <= attached count for that
// kind) are re-tokenised, so prose that happens to read "Image 1" is untouched.
