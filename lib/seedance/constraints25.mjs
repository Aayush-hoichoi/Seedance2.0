...
//
// Not just 2.5: the 2.0 family enforces the same 4–30s edit-source window,
// but where 2.5 rejects with a clear constraint error, 2.0 accepts the task
// and fails mid-render with an opaque "unexpected internal error" (verified:
// a 1.5s clip failed 7/7 background-replacement runs, Sep 2026, while 4s+
// clips with identical settings succeeded) — so those users need the warning
// most.
const EDIT_WARN_KINDS = ['full_2_5', 'full', 'fast', 'mini', 'full_sensitive'];

export function editClipDurationInvalid(durationSec) {
    return Number.isFinite(durationSec)
        && (durationSec < EDIT_CLIP_MIN_SEC || durationSec > EDIT_CLIP_MAX_SEC);
}

export function editClipWarning(modelKind, durationSec, name = 'This clip') {
    if (!EDIT_WARN_KINDS.includes(modelKind) || !Number.isFinite(durationSec)) return null;
    if (!editClipDurationInvalid(durationSec)) return null;
    const len = `${durationSec.toFixed(1)}s`;
    return `${name} is ${len} — Seedance can only EDIT clips of ${EDIT_CLIP_MIN_SEC}–${EDIT_CLIP_MAX_SEC}s. It still works as a style/motion reference, but a prompt that asks to change/remove/replace something in it will fail.`;
}
