...
// pills). Nothing else — the prompt text and the attached references live in
// draftMemory.mjs, under their own key and scoped per project. Two files
// because those references are inline base64 and can approach the origin's
// whole storage budget: a quota failure there must never cost these few
// hundred bytes, which have always survived a reload.
//
