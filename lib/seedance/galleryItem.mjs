...
export function toItem(r) {
    ...
    // Every stored image, not just index 0 — a 4-image job used to surface a
    // single image in history. Falls back to image_key for rows read before
    // the view exposed `images`.
    const imageUrls = isImage
        ? (() => {
            const urls = imageUrlsFromResult({ images: r.images });
            return urls.length ? urls : [presignKey(r.image_key)].filter(Boolean);
        })()
        : [];
    return {
        taskId: r.task_id,
        ...
        imageUrl: isImage ? (imageUrls[0] || null) : null,
        imageUrls: imageUrls.length > 1 ? imageUrls : null,
        exrArchiveKey: r.exr_archive_key || null,
        exrResolution: r.exr_resolution || null,
        exrUrl: r.exr_archive_key ? (presignKey(r.exr_archive_key) || r.exr_url || null) : (r.exr_url || null),
    };
}
