...
// Green Screen → Mannequin gate: fraction of chroma-green pixels in a frame
// sampled from mid-clip. A real green-screen backdrop fills a large share of
// the frame even with the performer in front; normal footage doesn't.
// ponytail: one sampled frame, simple g-dominance test — sample more frames if
// users defeat it with a single green-lit shot.
export function greenScreenShare(file) {
    return withObjectUrl(file, (url) => new Promise((resolve, reject) => {
        const video = document.createElement('video');
        video.preload = 'auto';
        video.muted = true;
        video.onloadedmetadata = () => {
            video.onseeked = () => {
                ...
                const h = (canvas.height = Math.max(1, Math.round((64 * video.videoHeight) / video.videoWidth) || 36));
                ...
                for (let i = 0; i < data.length; i += 4) {
                    ...
                    if (g > 90 && g > r * 1.4 && g > b * 1.4) green++;
                }
                resolve(green / (data.length / 4));
            };
            video.currentTime = Number.isFinite(video.duration) ? Math.min(video.duration / 2, 5) : 0;
        };
        video.onerror = () => reject(new Error('Could not read this video.'));
        video.src = url;
    }));
...
// omitting it keeps the conservative 2.0 spec. taskType relaxes 2.5's floor
// to 2s for a DECLARED reference task (see videoLimitsFor).
export async function validateMediaFile(kind, file, modelKind = null, taskType = null) {
    ...
    try {
        ...
        if (kind === 'video') {
            ...
            return { error: validateVideoMetadata(meta, modelKind, taskType), meta };
        }
        if (kind === 'audio') {
            const meta = await inspectAudio(file);
            return { error: validateAudioMetadata(meta, modelKind), meta };
        }
    } catch (e) {
