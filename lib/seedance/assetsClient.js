...
// Asset WRITES share a tight account-wide per-minute quota: a probe of 4
// parallel CreateAssets got one 200 and three immediate 429
// QuotaWriteQPMExceeded. Reads (GetAsset/List*) are unaffected. Chain writes
// so one submit (4 refs) or a batch delete never self-bursts; the backoff
// above absorbs cross-user overlap on the shared account.
...
// minutes, making 1h a generous garbage threshold (Reuse doesn't need pool
...
// Cross-submit registration cache. CreateAsset quota is the studio's scarcest
// resource (a handful per minute, account-wide — QuotaWriteQPMExceeded), so a
// ref that already registered must never spend another create: iterating on
