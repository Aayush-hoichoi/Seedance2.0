...
import { ASSET_TTL_HOURS } from './assetTtl.mjs';
...
// Serializing stopped the parallel burst but not the RATE: 120 QPM is ~2
// writes/second, and back-to-back sequential calls run at whatever the round
// trip allows (3-5/s), which still trips the bucket. Hold a floor gap between
// writes so the chain paces itself under the cap instead of just queueing at it.
const WRITE_GAP_MS = 600;
...
function callAsset(action, payload) {
    ...
    writeChain = run.then(() => sleep(WRITE_GAP_MS), () => sleep(WRITE_GAP_MS));
    return run;
}
...
export async function ensureUploadGroup(project = null) {
    ...
    // Cache the in-flight PROMISE, not the resolved id. resolveMediaRefs
    // registers every reference with Promise.all, so attaching four references
    // put four callers here before any had filled the cache: all four saw "no
    // group", all four created one, and production ended up with four identical
    // "Seedance Studio · test3 #22" groups. That is 4 wasted CreateAssetGroup
    // writes on top of the 4 CreateAsset ones, in a single burst, against the
    // account-wide 120 QPM zero-burst write quota — which is exactly the
    // "provider is rate-limiting new reference registrations" users hit.
    // Sharing the promise makes concurrent callers await one lookup.
    ...
    // Evict on failure so a transient error doesn't poison every later submit
    // with a rejected promise.
    uploadGroupCache.set(name, pending);
    pending.catch(() => uploadGroupCache.delete(name));
    return pending;
}
...
// live until it ages out. The window is ASSET_TTL_MINUTES (see assetTtl.mjs):
// a submit only needs its asset while ModelArk creates the task, so holding
// them for an hour just kept the pool full (Reuse doesn't need pool assets —
// it re-presigns from tosKey).
// This copy only runs while a studio tab is open; assetsServer.js carries the
// server-side twin that runs without one.
export async function cleanupOldAssets({ maxAgeHours = ASSET_TTL_HOURS, maxDeletes = 40 } = {}) {
    const groups = (await listGroups('AIGC')).filter((g) => g.name.startsWith(UPLOAD_GROUP_NAME));
    const cutoff = Date.now() - maxAgeHours * 3600 * 1000;
    let freed = 0;
    for (const g of groups) {
        // Capped: deletes and registrations share one account-wide write quota,
        // so an uncapped sweep of a backlogged pool spends minutes of it and
        // starves the upload that triggered the sweep. The rest ages out into
        // the next run — nothing is lost by going slower.
        if (freed >= maxDeletes) break;
        const assets = await listAssets(g.id, 'AIGC');
        const stale = assets
            .filter((a) => a.createdAt && Date.parse(a.createdAt) < cutoff)
            .slice(0, maxDeletes - freed);
...
// TTL sweep frees nothing (a burst filled the pool with fresh assets), evict
...
// Same discipline one level up: GetAsset THROWS for an id that no longer
// exists, and it throws for a throttle or an outage too — one channel, very
// different meanings. A caller deciding whether to DROP a user's attached
// reference may only act on the first, so this asks for proof of death rather
// than treating any failure as one. Wrong either way is bad, but silently
// deleting someone's reference because the network blipped is the worse half.
export function isAssetGone(message = '') {
    return /not\s*found|does\s*not\s*exist|no\s*such|deleted/i.test(message);
}
...
export async function createWithQuotaRecovery(create, cleanup = cleanupOldAssets) {
    try {
        ...
        // The default window, not a wider one: routine sweeps already ran at
        // the TTL, so asking for anything older here just spends a round-trip
        // at the exact moment the pool is under pressure.
        const freed = await cleanup();
...
export async function resolveMediaRefs(mediaItems, register = registerAssetFromUrl, verify = getAsset) {
    return Promise.all(mediaItems.map(async (m) => {
        ...
        if (cached?.settled) {
            ...
            // assetId-less entry: the sensitive-endpoint raw-URL fallback (its
            // register wrapper caches { url, assetId: null } when the Library
            // flagged the clip). Nothing to verify — keep the raw URL and skip
            // re-registering, which would just re-fail the same ~30s scan.
            if (prev && !prev.assetId) return { ...m };
