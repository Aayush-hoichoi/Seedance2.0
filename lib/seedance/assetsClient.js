...
export async function resolveMediaRefs(mediaItems, register = registerAssetFromUrl) {
    // De-dupe identical source URLs within one submit so the same file (e.g. the
    // same portrait dropped in two slots) registers once, not once per slot —
    // halving pool use and verification time. Keyed on kind+url minus its query
    // (presigned params vary per signing).
    ...
    return Promise.all(mediaItems.map(async (m) => {
        ...
        if (!inflight.has(key)) inflight.set(key, register({ url: m.url, kind: m.kind }));
        const asset = await inflight.get(key);
