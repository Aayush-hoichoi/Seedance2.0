...
export async function getAsset(id) {
    ...
    const r = data?.Result || {};
    // Error is only set on Status:Failed and is the ONLY place the real reason
    // shows up (e.g. InputVideoSensitiveContentDetected) — carry it through.
    return { id: r.Id, status: r.Status, name: r.Name, previewUrl: r.URL, groupId: r.GroupId, error: r.Error?.Message || r.Error?.Code || null };
}
...
// Group name is keyed by project id for stability across renames; the legacy
// single "Seedance Studio" group is still used when no project is given
// (pre-gateway / no-project setups). Cached per project id.
...
export function uploadGroupName(project) {
    ...
    return `Seedance Studio${label} #${project.id}`;
}
...
export async function pollAssetActive(id, { intervalMs = 3000, maxAttempts = 40, onStatus } = {}) {
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
        ...
        if (asset.status === 'Failed') throw new Error(asset.error || 'Asset verification failed — check it meets the format/size/content rules.');
...
// the user's own library and are never touched. Submits delete their own
// assets when every batch job finishes, so anything the sweep finds is
// leakage from pre-deploy tabs or tabs closed mid-render: renders take
// minutes, making 2h a generous garbage threshold (Reuse doesn't need pool
// assets — it re-presigns the source from tosKey).
export async function cleanupOldAssets({ maxAgeHours = 2 } = {}) {
...
export async function resolveMediaRefs(mediaItems, register = registerAssetFromUrl) {
    return Promise.all(mediaItems.map(async (m) => {
        ...
        const asset = await register({ url: m.url, kind: m.kind });
...
// The asset ids resolveVideoRefs registered for THIS submit: the resolved item
// is a new object (identity changed) carrying an assetId. Library picks arrive
// as asset:// and pass through by identity, so they are never included — only
// throwaway registrations, safe to delete once their generation finishes.
export function newlyRegisteredAssetIds(before, after) {
    return after.filter((m, i) => m !== before[i] && m?.assetId).map((m) => m.assetId);
}
...
export function registerAssetCached({ url, kind, project = null }) {
    const key = `${project?.id ?? 'default'}:${url.split('?')[0]}`;
    if (!assetMemo.has(key)) {
        const p = registerAssetFromUrl({ url, kind, project }).catch((e) => { assetMemo.delete(key); throw e; });
        assetMemo.set(key, p);
    }
