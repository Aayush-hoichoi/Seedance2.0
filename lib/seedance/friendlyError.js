...
// Appended to every moderation-flag message: the "Sensitive Content" model in
// the picker is a dedicated endpoint whose moderation accepts production
// footage the shared models flag. Gated — users without the grant can request
// access from the picker like any other gated model.
const SENSITIVE_HINT = ' Or switch the model to “Sensitive Content” (request access from the model picker if it’s locked) and generate again.';
...
const RULES = [
    ...
    },
    {
        ...
        msg: 'The provider’s moderation rejected the source video (flagged as sensitive). Nothing in the prompt will fix it — trim or swap the clip, or reframe/crop the flagged shot and re-upload.' + SENSITIVE_HINT,
    },
    {
        ...
        msg: 'The provider flagged a reference image as containing a real person. Swap that image or crop the face out, then generate again.' + SENSITIVE_HINT,
    },
    {
    ...
    {
        // OUTPUT moderation for copyright on the generated AUDIO — the
        // soundtrack resembled known music. References are not at fault.
        re: /audio.*copyright/i,
        msg: 'The generated soundtrack was flagged for possible music copyright. Turn Audio off (or drop music/chant cues from the prompt) and generate again.',
    },
    {
    ...
    {
        re: /sensitive/i,
        msg: 'The provider flagged the prompt or reference media as sensitive content. Adjust the prompt or swap the media and try again.' + SENSITIVE_HINT,
    },
    {
        ...
        msg: 'The source media didn’t pass the provider’s verification and no reason was returned — this is usually a moderation flag on the content. Trim or swap the clip, or re-crop the flagged shot, and re-upload.' + SENSITIVE_HINT,
    },
    {
