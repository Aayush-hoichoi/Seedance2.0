...
export const MODES = [
    ...
    },
    {
        ...
        hint: 'Upload a GREEN-SCREEN performance video — the studio first generates its silent white-mannequin motion twin, then automatically drives your character (from the reference images) with it. You get BOTH results in history: the mannequin video and the final output, playing side by side.',
        requiresText: true,
        // Stage 2 is a normal Mannequin generation — the user's prompt is
        // enhanced with the mannequin brief up front, at click time.
        enhanceStyle: 'mannequin',
        silentSource: true,
        // Flags the two-stage chain in SeedanceStudio: stage 1 converts the
        // green-screen video into the mannequin with a fixed brief, and its
        // completion auto-launches stage 2 with the user's images + prompt.
        autoMannequin: true,
        media: [
...
export const MANNEQUIN_SOURCE_PROMPT = `MANNEQUIN SOURCE CONVERSION — GREEN-SCREEN PERFORMANCE → SILENT WHITE MANNEQUIN

Objective: replace the performer in Video 1 ENTIRELY with a featureless matte-white mannequin performing the exact same motion, on a neutral studio backdrop. This clip is a MOTION SOURCE — nothing of the performer's identity may survive.

Mannequin:
- A smooth, seamless, matte-white articulated mannequin: blank face with no eyes, nose, mouth, hair or expression; no clothing, no textures, no logos.
- The mannequin performs Video 1's motion EXACTLY: body movement, arm, hand and finger gestures, head orientation, posture, weight shifts, and the precise timing, sequence, pacing and rhythm of every movement. Do not add, remove, smooth, exaggerate or reinterpret any movement.

Background:
- Replace the green screen with a plain, evenly lit, neutral light-grey studio background. No props, no set, no visible lighting rig — only the mannequin's natural soft contact shadow.
