...
export const MODELS = [
    ...
    //               -> 1080p re-enabled
    //   2026-09-22  4k re-probed: "not supported for this account and model"
    //               -> supports4k stays false; the studio flag that briefly
    //               shipped true was reverted (users hit the rejection at
    //               submit, after pricing + reservation)
    ...
    // this to false if the configured 2.5 endpoint rejects the tier. Same for
    // 4k in reverse: the refusal is account-worded, so re-probe before enabling.
...
// Any whole second in [4, max] is valid — the picker is a slider + free
// number box (PromptBar DurationControl), not a fixed stop list.
