...
export const MODELS = [
    ...
    // Same tier/pricing as Seedance 2.0 (kind 'full' — identical backing model).
    // `skipAssetLibrary`: refs go to the endpoint as raw TOS URLs instead of
    // through the Asset Library, whose ACCOUNT-level verification scan is what
    // rejects flagged source media before this endpoint ever sees it.
    { id: SENSITIVE_MODEL_ID, name: 'Sensitive Content', kind: 'full_sensitive', gated: true, supports1080p: true, supports4k: true, supportsReference: true, skipAssetLibrary: true },
];
