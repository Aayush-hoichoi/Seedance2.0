...
export function savePromptRecord({ taskId, userPrompt, generatedPrompt, style }) {
    // Best-effort: history UX must never block or fail a generation.
    return fetch('/api/seedance/prompts', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ taskId, userPrompt, generatedPrompt, style }),
    }).catch(() => null);
}

// taskIds → { [taskId]: { task_id, style, user_prompt, generated_prompt } }
