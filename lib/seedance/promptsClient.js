...
// saves the user's raw prompt + the enhancer-generated brief per taskId, and
// fetches them back so any browser can show the comparison tabs.

export function savePromptRecord({ taskId, userPrompt, generatedPrompt, style, refs = null, projectId = null }) {
    // Best-effort: history UX must never block or fail a generation.
    return fetch('/api/seedance/prompts', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ taskId, userPrompt, generatedPrompt, style, refs, projectId }),
    }).catch(() => null);
}
