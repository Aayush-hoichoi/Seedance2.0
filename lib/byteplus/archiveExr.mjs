...
const HOST_RE = /\.(volces\.com|bytepluses\.com|volcvideo\.com)$/i;
...
export async function archiveExr({ url, taskId }) {
    ...
    if (!put.ok) throw new Error(`EXR archive upload failed (${put.status}).`);
    return { key, url: presignGetUrl({ host, path, ak, sk, expiresSec: 604800 }), durable: true, bytes: bytes.length };
}
