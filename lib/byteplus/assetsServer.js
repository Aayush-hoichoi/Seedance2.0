...
import { ASSET_TTL_HOURS } from '../seedance/assetTtl.mjs';
...
// writes/second, and a sweep's back-to-back deletes run at whatever the round
// trip allows, holding the account-wide quota for as long as the sweep lasts —
// which is what made a user's own upload trigger the storm that then throttled
// their next reference. Pace the chain under the cap. Overridable for tests.
const WRITE_GAP_MS = Number(process.env.ARK_WRITE_GAP_MS) || 600; // a junk override must not disable the brake
...
export async function cleanupOldAssets({ maxAgeHours = ASSET_TTL_HOURS, maxDeletes = 40 } = {}) {
    ...
    for (const group of groups) {
        ...
        const stale = assets
            ...
            // Capped per run: deletes spend the SAME account-wide write quota as
            // the registrations users are waiting on, so a backlogged pool must
            // not be drained in one go — the remainder ages out next window.
            .slice(0, maxDeletes - freed);
        // Deletes are serialized AND spaced by callAsset's write chain, so a
        // sweep paces itself under the quota rather than bursting through it.
...
export function sweepAssetsIfDue({ now = Date.now(), maxAgeHours = ASSET_TTL_HOURS } = {}) {
