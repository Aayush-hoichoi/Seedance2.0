...
function enhancementBody(videoUrl, options = {}, overrides = {}) {
    let configured = {};
    ...
    const selected = normalizeExrOptions(options, { strict: true });
    return {
        scene: 'common',
        bitrate_level: 'high',
        ...
        ...configured,
        tool_version: bytePlusToolVersion(selected.tier),
...
export function buildEnhancementRequest({ videoUrl = 'queued-source', options = {} } = {}) {
    const configuredDefaults = {
        ...EXR_DEFAULT_OPTIONS,
        resolution: process.env.BYTEPLUS_VOD_EXR_RESOLUTION?.trim() || EXR_DEFAULT_OPTIONS.resolution,
        fps: Number(process.env.BYTEPLUS_VOD_EXR_FPS) || EXR_DEFAULT_OPTIONS.fps,
    };
    return enhancementBody(videoUrl, normalizeExrOptions({ ...configuredDefaults, ...options }, { strict: true }));
}
...
export async function submitEnhancement({ videoUrl, requestBody = null }) {
    ...
    const { _gallery, _billing, _upscale, ...providerRequestBody } = requestBody || {};
    // Upscale jobs carry the exact provider fields the user picked; the EXR
    // defaults (16-bit, fps, output_format) must not leak into them.
    const payload = _upscale
        ? { ...providerRequestBody, video_url: videoUrl }
        : enhancementBody(videoUrl, {}, providerRequestBody);
...
export async function pollEnhancement(providerTaskId) {
    ...
    // BytePlus documents no progress percent today; pass one through if it
    // ever appears so the UI's bar upgrades from estimate to fact for free.
    const progress = Number(task.progress ?? task.percent ?? task.progress_percent);
    return { status: 'processing', progress: Number.isFinite(progress) ? progress : null };
}
