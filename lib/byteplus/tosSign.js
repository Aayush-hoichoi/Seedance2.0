...
// Query-presigned PUT URL (default 1h) for direct browser → TOS uploads.
// Pass the exact content-type the browser will set when calling fetch PUT.
export function presignPutUrl({ host, path, contentType, ak, sk, expiresSec = 3600, date = new Date() }) {
    const { dateStr, dtStr } = stamps(date);
    const scope = `${dateStr}/${TOS_REGION}/${SERVICE}/request`;
    ...
    const params = {
        ...
        'X-Tos-SignedHeaders': signedHeaderNames,
    };
    ...
    const canonicalHeaders = contentType
        ...
        : `host:${host}\n`;
    const canonical = ['PUT', path, qs, canonicalHeaders, signedHeaderNames, UNSIGNED].join('\n');
    const sts = ['TOS4-HMAC-SHA256', dtStr, scope, sha256Hex(canonical)].join('\n');
    const signature = crypto.createHmac('sha256', signingKey(sk, dateStr)).update(sts, 'utf8').digest('hex');
