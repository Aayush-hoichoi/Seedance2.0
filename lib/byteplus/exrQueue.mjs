...
// status 'reserving' lets the caller hold the job out of the worker's reach
// until its budget reservation lands (the worker only claims queued|processing).
export async function enqueueExrJob(sql, { userId, projectId = null, sourceUrl, requestBody, status = 'queued' }) {
    const [row] = await sql`INSERT INTO exr_jobs
        (project_id, user_id, source_url, request_body, status, run_after)
        VALUES (${projectId}, ${userId}, ${sourceUrl}, ${JSON.stringify(requestBody || {})}, ${status}, now())
        RETURNING *`;
    return row;
}
...
// The two live inputs exrProgress (exrProgress.mjs) can't know on its own:
// how many jobs run/wait ahead of this one, and how long jobs of this kind
// (EXR vs upscale) ACTUALLY take — the median of the last 50 successes.
export async function exrProgressContext(sql, job) {
    const upscale = !!job?.request_body?._upscale;
    const [row] = await sql`SELECT
        (SELECT count(*)::int FROM exr_jobs q
          WHERE q.status = 'processing'
             OR (q.status = 'queued' AND q.created_at < ${job.created_at})) AS ahead,
        (SELECT percentile_cont(0.5) WITHIN GROUP (ORDER BY extract(epoch FROM (s.finished_at - s.started_at)))
           FROM (SELECT started_at, finished_at FROM exr_jobs
                 WHERE status = 'succeeded' AND started_at IS NOT NULL AND finished_at IS NOT NULL
                   AND (request_body ? '_upscale') = ${upscale}
                 ORDER BY finished_at DESC LIMIT 50) s) AS typical_secs`;
    return {
        queueAhead: row?.ahead ?? 0,
        typicalMs: row?.typical_secs != null ? Math.round(Number(row.typical_secs) * 1000) : null,
    };
}

// Same medians for the admin queue, where one call must cover every row.
export async function exrTypicalDurations(sql) {
    const rows = await sql`SELECT (request_body ? '_upscale') AS upscale,
            percentile_cont(0.5) WITHIN GROUP (ORDER BY extract(epoch FROM (finished_at - started_at))) AS secs
        FROM exr_jobs
        WHERE status = 'succeeded' AND started_at IS NOT NULL AND finished_at IS NOT NULL
        GROUP BY 1`;
    const out = { exr: null, upscale: null };
    for (const r of rows) out[r.upscale ? 'upscale' : 'exr'] = Math.round(Number(r.secs) * 1000);
    return out;
}
...
export async function getExrQueue(sql, { status = null, limit = 200 } = {}) {
    ...
    const [items, counts] = await Promise.all([
        ...
        sql`SELECT status, (request_body ? '_upscale') AS upscale, count(*)::int AS count
            FROM exr_jobs GROUP BY 1, 2 ORDER BY status`,
    ]);
    return { items, counts };
}
...
export async function retryExrJob(sql, id) {
    const [row] = await sql`UPDATE exr_jobs
        ...
        WHERE id = ${id} AND status IN ('failed', 'cancelled')
          -- Budgeted jobs (upscale, and EXR since it gained reservations) are
          -- not retryable: their reservation is already closed, so a retry
          -- would run unbudgeted. The user resubmits. Legacy unbudgeted EXR
          -- jobs (no _billing.toolId) stay retryable.
          AND NOT (request_body -> '_billing' ? 'toolId')
        RETURNING *`;
    return row || null;
}

// Claim one succeeded-but-not-durable job so the worker can upgrade its
// provider URL to a durable archive copy in the background. The short lease
// only prevents concurrent duplicate downloads — a job whose upgrade attempt
// was killed mid-copy is claimable again as soon as the lease lapses.
export async function claimNextArchiveUpgrade(sql) {
    const [rows] = await sql.transaction([sql`
        WITH next_job AS (
            SELECT id
            FROM exr_jobs
            WHERE status = 'succeeded'
              ...
              AND result->>'archiveError' IS NULL
              AND coalesce((result->>'archiveAttempts')::int, 0) < 5
              AND (lease_until IS NULL OR lease_until < now())
        ...
        UPDATE exr_jobs AS j
        SET lease_until = now() + make_interval(secs => ${EXR_LEASE_SECONDS}), updated_at = now()
        FROM next_job
        WHERE j.id = next_job.id
        RETURNING j.*
    `]);
