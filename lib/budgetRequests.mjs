...
import { overCommitsProject } from './gateway/db.js';
...
async function requestContext(sql, { projectId, userId }) {
    ...
    const models = await sql`SELECT m.id, m.display_name, m.category, v.kind
        ...
        WHERE m.active = true ORDER BY (m.category = 'tool'), m.category, m.display_name`;
...
export async function createBudgetRequest({ projectId, user, modelId, quality, increaseAmount, reason, sql: providedSql = null }) {
    ...
    // 'All models' stays generation models only — a tool budget is always
    // requested for that one tool, and tools have no quality ladder.
    const selectedModels = modelId === ALL_MODELS
        ? context.models.filter((model) => model.category !== 'tool')
        : context.models.filter((model) => model.id === modelId);
    if (!selectedModels.length) throw new Error('Select an active model.');
    const isTool = modelId !== ALL_MODELS && selectedModels[0].category === 'tool';
    if (modelId === ALL_MODELS && !ALL_MODEL_QUALITIES.includes(quality)) {
        throw new Error('Select a valid all-model quality level.');
    }
    const caps = isTool ? {} : Object.fromEntries(selectedModels.map((model) => [model.id, qualityCap(model.id, quality)]));
    if (!isTool) {
...
export async function decideBudgetRequest({ id, action, admin, policy = 'hard', reason = null, approvedAmount = null, sql: providedSql = null }) {
    ...
    // Approving funds a member budget, which is carved out of the project's
    // overall budget — the same ceiling the console enforces. `increase` is the
    // delta the statements below add on top of the member's existing cap.
    const overCommit = await overCommitsProject(sql, {
        projectId: req.projectId, userId: req.userId, type: 'usd', window: 'lifetime', nextLimit: increase,
    });
    if (overCommit) return { error: 'overall_cap', overCommit };

    const modelScope = req.modelId === ALL_MODELS ? null : req.modelId;
    // A tool budget (tool:exr, tool:upscale) funds the quota scope only: tools
    // have no quality ladder and their access lives outside model grants, so
    // the per-model resolution writes below are skipped for them.
    const models = await sql`SELECT id, category FROM models WHERE active = true ORDER BY id`;
    const isTool = modelScope != null && models.some((model) => model.id === modelScope && model.category === 'tool');
    const selected = modelScope
        ? models.filter((model) => model.id === modelScope)
        : models.filter((model) => model.category !== 'tool');
    if (!selected.length) return { error: 'model_inactive' };
    const caps = req.qualityCaps || {};
    const resolvedCaps = isTool ? {} : Object.fromEntries(selected.map((model) => [
        model.id,
        caps[model.id] ?? qualityCap(model.id, req.quality),
    ]));
    if (!isTool && Object.values(resolvedCaps).some((cap) => !cap)) return { error: 'quality_unconfigured' };
    ...
    for (const model of (isTool ? [] : selected)) {
