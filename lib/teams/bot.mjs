...
// A Teams address and an app login are NOT the same identity, and assuming
// they were sent zero cards for a day: TEAMS_ADMIN_EMAILS held work addresses
// (aayush@hoichoi.tv) while users.email holds the account they actually sign in
// with (aayushkumarhigh@gmail.com), so every lookup missed and every send
// silently skipped.
//
// So an entry is either:
//   aayush@hoichoi.tv                              — both identities are the same
//   aayush@hoichoi.tv=aayushkumarhigh@gmail.com    — deliver to the first,
//                                                    authorise as the second
//
// teamsEmail is what Graph resolves to a chat; appEmail is what must match an
// admin in users.email for the approve/deny links to be signable.
export function approverRecipients() {
    return (process.env.TEAMS_ADMIN_EMAILS || '')
        ...
        .filter(Boolean)
        .map((entry) => {
            const [teamsEmail, appEmail] = entry.split('=').map((part) => part.trim()).filter(Boolean);
            return { teamsEmail, appEmail: appEmail || teamsEmail };
        })
        .filter((r) => r.teamsEmail);
}

// The delivery addresses alone — kept for the config check and existing callers.
export function approverEmails() {
    return approverRecipients().map((r) => r.teamsEmail);
}
