...
- If the user quotes dialogue lines or names the spoken language (e.g. Hindi), carry them verbatim into the audio section as the exact lines/language heard in the source video and required in the output.
...
- Reproduce every spoken line exactly as it is heard in Video 1, in the SAME language it is spoken in. Never translate the dialogue and never switch its language.
...
- Every dialogue line, word for word, in its original language.
...
Everything else must remain exactly the same as the original 10-second source video. The output audio must reproduce the source video's original soundtrack exactly — the same dialogue lines in the same language, the same voices, the same timing — perfectly lip-synced, with nothing added and nothing removed.`;
...
- Reproduce every spoken line word for word, in the SAME language it is spoken in — never translate it or switch its language.
