...
// `onCreated` / `onDecided` are post-commit side effects (the Teams cards).
// Injected rather than imported so this module stays network-free under
// `node --test`, and deliberately never fatal: by the time they run the request
// or decision is already durably recorded, so a notification failure must not
// turn a successful response into an error the caller sees.
export function createBudgetRequestRouteHandlers({ authenticate, loadContext, createRequest, onCreated = null }) {
    return {
        async GET(request) {
        ...
        async POST(request) {
            ...
            try {
                ...
                if (onCreated) {
                    try { await onCreated(result); } catch (err) { console.error('[budget] onCreated failed:', err?.message); }
                }
...
export function createBudgetDecisionRouteHandler({ authenticate, canReview, decideRequest, onDecided = null }) {
    return async function POST(request, { params }) {
        ...
        try {
            const result = await decideRequest({
                id, action, admin, policy: body?.policy, reason: body?.reason,
            ...
            if (result.error === 'amount') return json({ error: 'The approved amount must be a number greater than zero.' }, { status: 400 });
            ...
            if (result.error === 'quality_unconfigured') return json({ error: 'Quality tiers are not configured for every selected model.' }, { status: 409 });
            if (result.error === 'requester_ineligible') return json({ error: 'The requester is no longer an active member of this project.' }, { status: 409 });
            if (result.error === 'overall_cap') return json({ error: `This would exceed the project's overall budget of $${result.overCommit.overallCap.toFixed(2)} — only $${result.overCommit.available.toFixed(2)} is unallotted. Raise the overall budget first.` }, { status: 409 });
            // A console decision must reach the Teams cards too — otherwise they
            // sit showing "pending" for a request that is settled. Awaited so a
            // failure is logged in context, never rethrown: the decision is
            // already committed and cannot be undone by a card that won't update.
            if (onDecided) {
                try { await onDecided({ id, action, admin, decision: result }); }
                catch (err) { console.error('[budget] onDecided failed:', err?.message); }
            }
            return json(result);
        } catch (error) {
