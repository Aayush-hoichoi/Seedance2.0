...
export async function getRequestsForUser(userId) {
    const sql = await getDb();
    if (!sql) return [];
    return sql`SELECT model_id, status, expires_at, project_id FROM model_access_requests WHERE user_id = ${userId}`;
}
...
export async function requestAccess(userId, email, modelId, note, projectId) {
    const sql = await getDb();
    ...
    // grant re-opens as a fresh request. `fresh` tells callers to notify.
    const [existing] = await sql`SELECT id, status, expires_at FROM model_access_requests
        WHERE user_id = ${userId} AND model_id = ${modelId} AND project_id = ${projectId} LIMIT 1`;
    const decision = reRequestDecision(existing);
    if (decision !== 'fresh') return { id: existing.id, status: decision, fresh: false };
    ...
    const rows = await sql`INSERT INTO model_access_requests (user_id, user_email, model_id, project_id, status, note, created_at)
        VALUES (${userId}, ${email}, ${modelId}, ${projectId}, 'pending', ${note ?? null}, now())
...
export async function listRequests() {
    const sql = await getDb();
    if (!sql) return [];
    return sql`SELECT r.id, r.user_id, r.user_email, r.model_id, r.status, r.note,
...
export async function listActiveGrants() {
    ...
    return sql`SELECT r.id, r.user_id, r.user_email, r.model_id, r.decided_by,
            r.expires_at, r.project_id, p.name AS project_name
        FROM model_access_requests r
        LEFT JOIN projects p ON p.id = r.project_id
...
export async function setRequestStatus(id, status, decidedBy, expiresAt = null) {
    const sql = await getDb();
    if (!sql) throw new Error('Access store unavailable');
    // Approve only lands on a pending request — a stale duplicate Slack card
    // must not re-approve a decided one (or silently rewrite its expiry).
    ...
    const rows = await sql`UPDATE model_access_requests
        ...
        WHERE id = ${id} AND (${status} <> 'approved' OR model_access_requests.status = 'pending')
        RETURNING id, user_id, user_email, model_id, status, expires_at, project_id`;
    return rows[0] ?? null;
}
