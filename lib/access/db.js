...
import { queryUserGenerations, queryUserGenerationProjects } from './galleryQueries.mjs';
import { EXR_ACCESS_MODEL_ID } from '../byteplus/exrAccess.mjs';
...
export async function getApprovedModelIds(userId) {
    ...
    const rows = await sql`SELECT model_id FROM model_access_requests
        WHERE user_id = ${userId} AND model_id <> ${EXR_ACCESS_MODEL_ID} AND status = 'approved'
...
export async function getRequestsForUser(userId) {
    ...
    return sql`SELECT model_id, status, expires_at, project_id, max_resolution, pending_max_resolution
        FROM model_access_requests WHERE user_id = ${userId} AND model_id <> ${EXR_ACCESS_MODEL_ID}`;
}
...
// Workspace-wide position on the same current-month user leaderboard shown by
// the console's default "Top users" view. Only the caller's position and the
// size of the leaderboard leave this module; individual competitors and their
// spend stay private.
export async function getMonthSpendRank(userId, providedSql = null) {
    const sql = providedSql ?? await getDb();
    if (!sql) return null;
    const [row] = await sql`WITH user_spend AS (
            SELECT user_id,
                   SUM(COALESCE(cost_usd, est_cost_usd, 0)) AS cost_usd
            FROM billing_events
            ...
            SELECT user_id,
                   RANK() OVER (ORDER BY cost_usd DESC) AS spend_rank,
                   COUNT(*) OVER () AS user_count
            FROM user_spend
        )
        SELECT spend_rank::int, user_count::int
        FROM leaderboard
        WHERE user_id = ${userId}`;
    if (!row) return null;
    return { rank: Number(row.spend_rank), userCount: Number(row.user_count) };
}
...
// How listCreators() may order its rows. A whitelist, not an interpolated
// caller string — the clause is concatenated into the SQL text, so only these
// two fixed values may ever reach it.
//   'volume' — biggest contributors first; the admin roster reads as a leaderboard.
//   'recent' — most recently active first; what the gallery's creators sidebar
//              wants, so the list runs newest → oldest by activity instead of
//              scattering "5m ago" and "8/1/2026" through a count-ranked list.
// Both fall back through the other key, so ties stay deterministic.
...
// `last_at` skips failed generations for the same reason `generations` does:
// both describe the work the gallery actually shows, so a creator can't be
// sorted (or labelled) as freshly active on a generation that never rendered.
export async function listCreators({ order = 'volume' } = {}) {
    const sql = await getDb();
    if (!sql) return [];
    // sql.query() (plain text, no bind params here) rather than the tagged
    // template, because the ORDER BY clause varies by caller.
    return sql.query(`SELECT coalesce(u.id, s.user_id) AS id,
        ...
        FULL OUTER JOIN (
            SELECT user_id, max(user_email) AS user_email,
                count(*) FILTER (WHERE status <> 'failed') AS generations,
                max(created_at) FILTER (WHERE status <> 'failed') AS last_at
...
// `before` (ISO timestamp) pages further back: rows strictly older than it.
// The gallery route hands the last row's created_at back to the client as
// `nextBefore`, so the studio rail can walk an arbitrarily long history in
// 200-row pages instead of silently truncating at the first one.
export async function listUserGenerations(userId, limit = 200, before = null, projectId = null) {
    const sql = await getDb();
    if (!sql) return [];
    return queryUserGenerations(sql, { userId, limit, before, projectId });
}

// Project facets for one creator's gallery. Counts use the exact same visible
// row rules as listUserGenerations, so the selector never advertises deleted or
// failed work that the grid cannot show.
export async function listUserGenerationProjects(userId) {
    const sql = await getDb();
    if (!sql) return [];
    return queryUserGenerationProjects(sql, { userId });
}

// The CALLER's liked generations only (the like flag is a shared mark on the
// task, so ownership scopes it: you see the hearts on your own work), newest
// first. Legacy unattributed tasks can't be proven yours, so they stay out.
export async function listLikedGenerations(userId, limit = 200) {
    const sql = await getDb();
    if (!sql) return [];
    return sql`SELECT p.task_id, p.user_prompt, p.generated_prompt, p.style, p.refs, p.liked,
        ...
        JOIN gallery_generations e ON e.task_id = p.task_id AND e.user_id = ${userId}
...
// Append one row to the engagement log (generation_events). Best-effort: a
// like/download is a UI nicety and its logging must never break the action, so
// this swallows every failure. Powers the dataset_samples signal + per-user
// attribution the old shared `liked` boolean never had. Caller passes an open
// `sql` (it already has one) so this adds no extra getDb() round-trip.
export async function recordGenerationEvent(sql, { taskId, userId = null, projectId = null, eventType }) {
    if (!sql || !taskId || !eventType) return;
    try {
        await sql`INSERT INTO generation_events (task_id, user_id, project_id, event_type)
            VALUES (${taskId}, ${userId}, ${projectId}, ${eventType})`;
    } catch (err) {
        console.error('[engagement] event log failed:', err.message);
    }
}
