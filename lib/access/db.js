...
export async function requestAccess(userId, email, modelId, note, projectId) {
    ...
    // one project. The WHERE guard makes re-requesting a no-op for an approved
    // grant — a stale UI click must never downgrade access an admin granted.
    const rows = await sql`INSERT INTO model_access_requests (user_id, user_email, model_id, project_id, status, note, created_at)
        ...
        RETURNING id, status`;
    // No row back = the guard skipped an already-approved grant.
    return { id: rows[0]?.id ?? null, status: rows[0]?.status ?? 'approved' };
}
