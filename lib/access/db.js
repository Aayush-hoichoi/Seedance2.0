...
export async function getRequestsForUser(userId) {
    const sql = await getDb();
    if (!sql) return [];
    return sql`SELECT model_id, status, expires_at FROM model_access_requests WHERE user_id = ${userId}`;
}
...
export async function listRequests() {
    const sql = await getDb();
    if (!sql) return [];
    return sql`SELECT id, user_id, user_email, model_id, status, note, decided_by, created_at, decided_at, expires_at
...
export async function setRequestStatus(id, status, decidedBy, expiresAt = null) {
    ...
    const rows = await sql`UPDATE model_access_requests
        ...
        WHERE id = ${id}
        RETURNING id, user_id, model_id, status, expires_at`;
    return rows[0] ?? null;
}
