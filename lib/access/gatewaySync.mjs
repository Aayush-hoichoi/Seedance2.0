...
// sql is injectable so the approve path can be exercised under `node --test`
// against a stub client — the same dependency-injection the pure engines in
// lib/gateway use. Production callers omit it and get the real connection.
export async function syncGatewayOverride({ action, row, admin, validUntil, sql: injectedSql = null }) {
    const sql = injectedSql ?? await getDb();
    if (!sql) return;
    ...
    if (action === 'approve') {
        // max_resolution rides the decided request row (the admin's granted
        // tier, possibly lower than asked); NULL = no quality cap.
        const [override] = await sql`INSERT INTO user_model_overrides
            (project_id, user_id, model_id, effect, max_resolution, valid_until, created_by, revoked_at)
            VALUES (${project.id}, ${row.user_id}, ${version.model_id}, 'allow', ${row.max_resolution ?? null}, ${validUntil ?? null}, ${admin.userId}, NULL)
            ON CONFLICT (project_id, user_id, model_id) WHERE source_request_id IS NULL
            DO UPDATE SET effect = 'allow', revoked_at = NULL, max_resolution = EXCLUDED.max_resolution, valid_until = EXCLUDED.valid_until, created_by = EXCLUDED.created_by
            RETURNING id`;
        await emitEvent(sql, {
            projectId: project.id, userId: row.user_id,
            type: 'access.granted', payload: { modelId: version.model_id, scope: 'user', effect: 'allow', maxResolution: row.max_resolution ?? null, validUntil: validUntil ?? null, via: 'access_request' },
        });
        await writeAudit(sql, {
            actorId: admin.userId, actorEmail: admin.email, action: 'override.allow',
            targetType: 'user_model_override', targetId: override.id,
            after: { projectId: project.id, userId: row.user_id, modelId: version.model_id, effect: 'allow', maxResolution: row.max_resolution ?? null, requestId: row.id },
        });
        return;
    }
