...
export async function cancelJob(sql, job, { reason = 'cancelled' } = {}) {
    ...
    await insertBillingEvent(sql, {
        eventType: 'release', generationId: job.id, orgId: job.org_id, projectId: job.project_id,
    ...
    if (updated.provider_task_id || updated.batch_job_name) {
        ...
        const auth = await resolveApiKey(sql, { providerId, orgId: job.org_id, projectId: job.project_id });
        if (auth) {
            if (updated.batch_job_name) await google.cancelBatch({ batchName: updated.batch_job_name, apiKey: auth.key });
            else await byteplus.cancel({ job: updated, apiKey: auth.key });
        }
    }

    await emitEvent(sql, {
        orgId: job.org_id, projectId: job.project_id, userId: job.user_id,
