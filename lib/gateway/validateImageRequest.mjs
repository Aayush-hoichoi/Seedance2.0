...
// ChatGPT Image 2.5 only — other models' adapters ignore both. Off-list values
// drop to null (provider/pricing defaults apply) rather than erroring, same
// posture as the ratio/size whitelists. xhigh/max quality exist upstream but
// are deliberately not accepted: quality scales the price, and those two are
// costly enough to wreck the flat per-tier rates.
const IMAGE_QUALITIES = new Set(['low', 'medium', 'high']);
const IMAGE_VARIANTS = new Set(['flare', 'sunburst']);
...
// maxRefImages is the per-model reference-image cap (Nano Banana Pro = 14, Flash
// = 3); callers pass it from constants.imageRefMax(modelId). Defaults to the
// conservative Flash cap so a caller that omits it can't over-accept.
export function sanitizeImageRequest(request, options = {}, maxRefImages = MAX_REF_IMAGES) {
    ...
    if (request.parts != null) {
        ...
        for (const p of request.parts) {
            ...
            refCount += 1;
            if (refCount > maxRefImages) return { error: `Attach at most ${maxRefImages} reference images.` };
    ...
    const quality = IMAGE_QUALITIES.has(options?.quality) ? options.quality : null;
    const variant = IMAGE_VARIANTS.has(options?.variant) ? options.variant : null;
    return { request: parts ? { prompt, parts } : { prompt }, imageCount, aspectRatio, imageSize, quality, variant };
}
