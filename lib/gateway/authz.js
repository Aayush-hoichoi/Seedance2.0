...
export async function gatewayContext({ projectId = null, permission = null } = {}) {
    ...
    // Workspace manager: a user who holds admin/manager on ANY project acts as
    // a manager across ALL projects — they see every project and manage its
    ...
    // A platform 'manager' role (assigned from the Users console) is a workspace
    ...
    if (!isPlatformAdmin && !isOrgManager) {
        const [row] = await sql`SELECT 1 FROM project_memberships m
            ...
            WHERE m.user_id = ${user.userId} AND p.archived_at IS NULL
    ...
    if (projectId != null) {
        ...
        else if (isOrgManager) role = 'manager'; // workspace managers act as manager on any project
