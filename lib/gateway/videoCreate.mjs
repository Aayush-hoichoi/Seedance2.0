...
import { MODELS, GATED_MODEL_IDS, RESOLUTIONS, resolutionWithinTier, supportedResolutionsFor, ratioIsInherited, INHERITED_RATIO } from '../seedance/constants.js';
import { OMNI_TASK_TYPES } from '../seedance/constraints25.mjs';
import { canUseModel } from '../access/decision.mjs';
import { estimateCost, budgetDuration } from '../seedance/pricing.mjs';
import { getDb } from '../db/neon.js';
import { effectiveAccess } from './access.mjs';
import { evaluateQuotas } from './quota.mjs';
import { activeQuotas, usageForQuotas, insertBillingEvent, reserveBillingEvent, emitEvent, workflowStyle, userWorkflowId } from './db.js';
import { PROJECT_CONCURRENCY } from './queueLogic.mjs';
import { composeStyledPrompt, applyStyledVideoPrompt, videoPromptOf } from './projectStyle.mjs';
...
// create call returns 200 with a task id, and the TASK then fails minutes later
// with "`duration` must be -1". Verified against the live API with an identical
// request, video reference and edit prompt:
//
//   duration=5   accepted at submit -> task FAILED  "`duration` must be -1"
//   duration=-1  accepted at submit -> task SUCCEEDED
//
// So reacting to the create RESPONSE can never work: response.ok is true and
// there is nothing to retry. An earlier fix did exactly that and was dead code.
// Retrying after the async failure would mean a second provider task against
// one job's reservation, plus the user sitting through a doomed render first.
// Decide before sending instead.
//
// An edit inherits the source clip's length, so when a video reference rides
// along on 2.5 we send -1 and let the model take the duration from the clip.
// The cost is duration control on 2.5 video-to-video, which is the price of
// never burning a render on a verdict we cannot predict.
...
export async function createVideoTask({ user, projectId = null, mode = null, styleLook = null, workflowId = null, request }) {
    ...
    const lowered = typeof request?.resolution === 'string'
        ? { ...request, resolution: request.resolution.toLowerCase() }
        : request;
    const modelId = lowered?.model;

    // omni_reference_task_type (doc revision 2026-09-04) declares the subtype
    // up front. A garbage value would only fail at the provider with an opaque
    // error, after a slot + reservation were taken — reject it here instead.
    const taskType = lowered?.omni_reference_task_type ?? 'auto';
    if (!OMNI_TASK_TYPES.includes(taskType)) {
        return { status: 400, body: { error: `omni_reference_task_type must be one of: ${OMNI_TASK_TYPES.join(', ')}.` } };
    }

    // See AUTO_DURATION above: on 2.5 a video reference means the model may
    // classify this as an edit while it renders, and an edit inherits the
    // source clip's length. Send -1 up front so that verdict cannot fail the
    // job after we have already reserved budget and taken a queue slot.
    // Scoped to 2.5 because that is where it was observed — 2.0 accepts a fixed
    // duration with a reference video today and is left alone.
    // A DECLARED subtype narrows the force: 'reference' and 'extend' both take
    // an explicit duration per the docs, so only 'edit' and 'auto' (where the
    // model may still pick edit) get the rewrite.
    const withVideoRef = hasVideoInput(lowered?.content);
    const hasFirstFrame = Array.isArray(lowered?.content)
        && lowered.content.some((c) => c?.role === 'first_frame' || c?.role === 'last_frame');
    const inheritsSourceDuration = MODELS.find((m) => m.id === modelId)?.kind === 'full_2_5'
        && withVideoRef
        && (taskType === 'auto' || taskType === 'edit')
        && lowered?.duration !== AUTO_DURATION;

    // Ratio is not ours either. The docs: for editing, extension and
    // first/last-frame, ratio "defaults to and only supports adaptive" and the
    // output follows the selected input. Sending 16:9 fails the task — one of
    // the three parameters production job 7002 was rejected on. A declared
    // 'reference' task has no ratio constraint (ratioIsInherited knows).
    const inheritsSourceRatio = ratioIsInherited({ modelId, hasVideoRef: withVideoRef, hasFirstFrame, taskType })
        && lowered?.ratio !== INHERITED_RATIO;

    const parsed = (inheritsSourceDuration || inheritsSourceRatio)
        ? {
            ...lowered,
            ...(inheritsSourceDuration ? { duration: AUTO_DURATION } : {}),
            ...(inheritsSourceRatio ? { ratio: INHERITED_RATIO } : {}),
        }
        : lowered;
    if (inheritsSourceDuration || inheritsSourceRatio) {
        const forced = [inheritsSourceDuration && 'duration=-1', inheritsSourceRatio && 'ratio=adaptive'].filter(Boolean).join(', ');
        console.log(`[byteplus] ${modelId}: input reference attached, sending ${forced} so the output can inherit from the source`);
    }
    ...
    // Project style memory (see lib/gateway/projectStyle.mjs). This sits below
    // resolveGateway because it needs the project row — which is why the
    // request is serialized here rather than above: `body` is consumed once, at
    // the provider fetch, so moving it down costs nothing.
    const userPrompt = videoPromptOf(parsed?.content);
    // An attached workspace workflow overrides the project's own style. No
    // explicit id → the user's stored attachment (users.workflow_id), so
    // MCP/API calls inherit it. A missing/deleted workflow falls back to the
    // project style.
    const effectiveWorkflowId = gw ? (workflowId ?? await userWorkflowId(gw.sql, user.userId)) : null;
    const attachedStyle = effectiveWorkflowId
        ? await workflowStyle(gw.sql, effectiveWorkflowId, { userId: user.userId, isAdmin: user.role === 'admin' })
        : null;
    const styled = composeStyledPrompt(userPrompt, attachedStyle ?? gw?.project?.style, { look: styleLook });
    const sent = styled.applied
        ? { ...parsed, content: applyStyledVideoPrompt(parsed.content, styled.prompt) }
        : parsed;
    ...
    if (gw) {
        ...
        const verdict = evaluateQuotas({
            ...
            estimate: { usd: estUsd ?? 0, video_seconds: budgetDuration(kind, parsed?.duration), requests: 1 }, ...usage,
        });
        if (!verdict.ok) {
    ...
    if (gw) {
        const requestBody = JSON.stringify({
            // The prompt the provider actually received. Until now the video
            // path persisted none — the only server-side record was whatever
            // the BROWSER later posted to seedance_prompts, which meant a
            // server-composed prompt would have been invisible in
            // generation_ledger and unanswerable after the fact ("did the style
            // fire on this shot?"). MCP and direct API callers wrote nothing at
            // all. Storing it here also matches what the image path has always
            // done.
            prompt: videoPromptOf(sent?.content),
            options: {
                ...
                style_look: styled.applied ? styled.look : null,
                style_version: styled.applied ? styled.version : null,
                style_workflow: styled.applied && attachedStyle ? Number(effectiveWorkflowId) : null,
            },
            est_cost_usd: estUsd,
            category: 'video',
        });
        ...
        try {
            reservation = await reserveBillingEvent(gw.sql, {
                ...
                userId: user.userId, modelId: gw.alias, modelVersionId: gw.versionId, providerId: 'byteplus',
                units: { video_seconds: budgetDuration(kind, parsed?.duration) }, estCostUsd: estUsd, pricingSnapshot: { basis: 'estimate' },
            });
        } catch (error) {
            try {
    ...
    if (response.ok && data?.id) {
        data = {
            ...data,
            jobId: job?.id ?? null, // MCP needs the gateway job id; data.id is the provider task id
            // Whether a style actually fired (and which), so clients can show
            // it honestly instead of assuming the attachment worked.
            style: styled.applied
                ? { look: styled.look, workflowId: attachedStyle ? Number(effectiveWorkflowId) : null }
                : null,
        };
        if (gw) {
