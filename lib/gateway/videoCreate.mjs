...
export async function createVideoTask({ user, projectId = null, mode = null, request }) {
    ...
    const inheritsSourceDuration = MODELS.find((m) => m.id === modelId)?.kind === 'full_2_5'
        && hasVideoInput(lowered?.content)
        && lowered?.duration !== AUTO_DURATION;
    const parsed = inheritsSourceDuration ? { ...lowered, duration: AUTO_DURATION } : lowered;
    if (inheritsSourceDuration) {
        console.log(`[byteplus] ${modelId}: video reference attached, sending duration=-1 so an edit can inherit the source length`);
    }
