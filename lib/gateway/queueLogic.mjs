...
// Capped so the tail attempts stay a sane wait rather than 4**n hours: 10s,
// 40s, then 60s per attempt. The cap is what keeps the whole no-handle
// ride-out inside its ~2 min budget — the queue only ticks once a minute, so
// the wall-clock a user sees is this total plus up to ~55s per retry.
export const BACKOFF_CAP_MS = Number(process.env.GATEWAY_BACKOFF_CAP_MS || 60_000);
...
// preview models get a small fixed compute pool.
//
// Only jobs with NO provider handle get the extra attempt: nothing billable
...
// generation twice.
//
// Capped at 4 (3 retries) on purpose: a 6-attempt ride-out reached the 600s
// backoff steps and left the user on a spinner for ~25 min before being told
// it failed — past the 15 min the studio itself waits (SeedanceStudio
// pollImageJob), so users only ever saw "Timed out waiting for the image",
// never the real reason. A slow success is worse than a prompt failure they
// can act on — switch to Nano Banana 2, or drop 4K to 2K.
export const MAX_ATTEMPTS_NO_HANDLE = Number(process.env.GATEWAY_MAX_ATTEMPTS_NO_HANDLE || 4);
export function maxAttemptsFor(job = {}) {
    return hasProviderHandle(job) ? MAX_ATTEMPTS : MAX_ATTEMPTS_NO_HANDLE; // 4 attempts: ~2 min backoff, ~3.5 min wall-clock
}
...
export function isRetryable(error = {}) {
    ...
    // Every undici transport fault is a lost connection, not a rejected
    // request — socket reset, headers/body timeout, connect timeout. ENETWORK
    // is our own label for one whose cause undici didn't give us. All of these
    // used to fail terminally on attempt 1 while the identical ETIMEDOUT case
    // retried, so a mid-flight drop lost the job outright.
    if (typeof error.code === 'string' && error.code.startsWith('UND_ERR_')) return true;
    return ['ETIMEDOUT', 'ECONNRESET', 'ECONNREFUSED', 'EAI_AGAIN', 'ENETWORK'].includes(error.code);
}
