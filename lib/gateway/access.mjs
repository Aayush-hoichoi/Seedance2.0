...
// → { allowed, rule, maxResolution } where rule names which precedence level
// decided, so the UI can explain itself ("denied by a user override until
// 2026-08-01"). maxResolution is the quality cap riding an allow override
// (null = uncapped); project grants and org defaults never cap. Known quirk:
// because allow-override beats project_grant, a capped user is MORE restricted
// than a teammate covered only by an uncapped project-wide grant.
export function effectiveAccess({ modelId, now, overrides = [], grants = [], defaultModelIds = [] }) {
    ...
    const allow = mine.find((o) => o.effect === 'allow');
    if (allow) return { allowed: true, rule: 'allow_override', maxResolution: allow.max_resolution ?? null };

    if (grants.some((g) => g.model_id === modelId && isActive(g, now))) {
        return { allowed: true, rule: 'project_grant', maxResolution: null };
    }
    if (defaultModelIds.includes(modelId)) return { allowed: true, rule: 'org_default', maxResolution: null };
...
// Studio front door: a plain member with no project memberships waits on
// /projects until an admin adds them. Admins/managers always enter — they see
// every project and can create one.
export function canEnterStudio(role, membershipCount) {
    return role === 'admin' || role === 'manager' || membershipCount > 0;
}
