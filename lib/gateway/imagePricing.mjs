...
//
// The NEXT_PUBLIC_ prefix on every override is LOAD-BEARING, not decoration.
// This module is imported by the prompt bar (app/seedance/estimateLabel.mjs →
// PromptBar.jsx, a client component) as well as by the gateway, and Next.js
// inlines ONLY NEXT_PUBLIC_* into the browser bundle. Under the old unprefixed
// names the browser silently fell back to the defaults below while the gateway
// reserved at the override — the user reading one price and being charged
// another, with nothing anywhere to show the two had parted company.
//
// For the same reason each name must stay a LITERAL member expression:
// process.env[someVariable] is not statically analysable, so Next cannot inline
// it and it reads undefined in the browser. Never collapse these into a loop.

// First argument that looks like a number wins; the last is the default.
// '' is rejected explicitly — Number('') is 0, so an env var that is present
// but blank used to price the model at FREE, which reserves nothing and takes
// the model outside budget enforcement entirely.
const num = (...vals) => {
    const fallback = vals.pop();
    const hit = vals.find((v) => v != null && v !== '' && !Number.isNaN(Number(v)));
    return hit == null ? fallback : Number(hit);
};

// A leftover unprefixed override would now be ignored — and being ignored
// silently is how the gateway ends up reserving less than the provider bills.
// Server-side only: these names are never present in the browser anyway.
if (typeof window === 'undefined' && typeof process !== 'undefined') {
    const legacy = Object.keys(process.env || {}).filter((k) => k.startsWith('PRICE_'));
    if (legacy.length) {
        console.warn(`[pricing] ignoring ${legacy.join(', ')} — rename to NEXT_PUBLIC_${legacy[0]} (etc.) so the studio quotes the same rate the gateway bills.`);
    }
}

export const IMAGE_RATES = {
    seedream_pro: { interactive: num(process.env.NEXT_PUBLIC_PRICE_SEEDREAM_PRO, 0.03) },
    nano_banana_pro: {
        interactive: num(process.env.NEXT_PUBLIC_PRICE_NANO_BANANA_PRO, 0.134),
        batch: num(process.env.NEXT_PUBLIC_PRICE_NANO_BANANA_PRO_BATCH, 0.067),
    },
    nano_banana_2: {
        interactive: num(process.env.NEXT_PUBLIC_PRICE_NANO_BANANA_2, 0.039),
        batch: num(process.env.NEXT_PUBLIC_PRICE_NANO_BANANA_2_BATCH, 0.0195),
    },
};
...
export const IMAGE_RATES_4K = {
    nano_banana_pro: {
        interactive: num(process.env.NEXT_PUBLIC_PRICE_NANO_BANANA_PRO_4K, 0.24),
        batch: num(process.env.NEXT_PUBLIC_PRICE_NANO_BANANA_PRO_4K_BATCH, 0.12),
    },
};
...
export const IMAGE_RATES_BY_SIZE = {
    chatgpt_image_2: {
        '1K': { interactive: num(process.env.NEXT_PUBLIC_PRICE_CHATGPT_IMAGE_2_1K, 0.03) },
        '2K': { interactive: num(process.env.NEXT_PUBLIC_PRICE_CHATGPT_IMAGE_2_2K, 0.05) },
        '4K': { interactive: num(process.env.NEXT_PUBLIC_PRICE_CHATGPT_IMAGE_2_4K, 0.08) },
    },
    ...
    chatgpt_image_2_5: {
        '1K': { interactive: num(process.env.NEXT_PUBLIC_PRICE_CHATGPT_IMAGE_2_5_1K, 0.05) },
        '2K': { interactive: num(process.env.NEXT_PUBLIC_PRICE_CHATGPT_IMAGE_2_5_2K, 0.16) },
        '4K': { interactive: num(process.env.NEXT_PUBLIC_PRICE_CHATGPT_IMAGE_2_5_4K, 0.32) },
    },
};
