...
import {
    ...
    activeQuotas, usageForQuotas, workflowStyle, userWorkflowId,
} from './db.js';
import { QUEUE_DEPTH_CAP } from './queueLogic.mjs';
import { sanitizeImageRequest } from './validateImageRequest.mjs';
import { composeStyledPrompt, applyStyledImagePrompt } from './projectStyle.mjs';
import { RESOLUTIONS, IMAGE_RESOLUTIONS, resolutionWithinTier, imageRefMax, clampImageResolution } from '../seedance/constants.js';
...
function estimateFor({ category, kind, mode, options = {} }) {
    if (category === 'image') {
        ...
        return { usd: imageCost(kind, mode, n, options.imageSize, options.quality) ?? 0, images: n, video_seconds: 0, requests: 1 };
    }
    ...
    return { usd, images: 0, video_seconds: budgetDuration(kind, options.duration), requests: 1 };
}
...
export async function enqueueGeneration({ user, projectId, modelId, request, options = null, priority = 'interactive' }) {
    ...
    if (routing.model.category === 'image') {
        ...
        resolvedOptions = {
            ...
            // ChatGPT Image 2.5 only; null for every other model. Persisted so
            // the reservation, the adapter and the settlement all read the same
            // whitelisted values (quality scales the price).
            quality: clean.quality,
            variant: clean.variant,
            // Some models couple ratio and resolution (GPT Image 2 refuses 4K at
            // 1:1, and anything above 1K without an explicit ratio) and reject the
            // task outright. Clamp HERE, not in the adapter: the persisted request
            // is what the reservation, the settlement and the pricing snapshot all
            // read, so a tier silently downgraded downstream would be billed at the
            // tier the user asked for rather than the one they received.
            imageSize: clampImageResolution(modelId, clean.aspectRatio, clean.imageSize),
        };
    }

    // Project style memory. Applied AFTER sanitize so the trust-boundary checks
    // (prompt present, length, refs) run on what the user actually sent, and
    // BEFORE insertJob so the composed prompt is what lands in jobs.request_body
    // — and therefore in generation_ledger and dataset_samples. If the persisted
    // prompt were not the sent prompt, the ledger would lie and the whole
    // evaluation loop would be measuring the wrong text.
    const { styleLook, styleWorkflowId, ...optionsWithoutStyleInput } = resolvedOptions;
    // An attached workspace workflow overrides the project's own style — the
    // user chose it explicitly. No explicit id → the user's stored attachment
    // (users.workflow_id), so MCP/API calls inherit it too. A missing/deleted
    // workflow falls back to the project style rather than erroring.
    const effectiveWorkflowId = styleWorkflowId ?? await userWorkflowId(sql, user.userId);
    const attachedStyle = effectiveWorkflowId
        ? await workflowStyle(sql, effectiveWorkflowId, { userId: user.userId, isAdmin: user.role === 'admin' })
        : null;
    const styled = composeStyledPrompt(requestBody.prompt, attachedStyle ?? project.style, { look: styleLook });
    if (styled.applied) {
        requestBody = applyStyledImagePrompt(requestBody, styled.prompt);
        ctx.requestBody = requestBody;
        // The caller's request key (styleLook) is dropped in favour of the
        // RESOLVED look, snake_cased. What matters downstream is which look
        // actually fired, not which was asked for — and gallery_generations
        // reads one expression for both the image and video pipelines.
        resolvedOptions = {
            ...optionsWithoutStyleInput,
            style_look: styled.look,
            style_version: styled.version,
            ...(attachedStyle ? { style_workflow: Number(effectiveWorkflowId) } : {}),
        };
    } else if (styleLook || styleWorkflowId) {
        resolvedOptions = optionsWithoutStyleInput; // asked for, did not apply — don't imply it did
    }
    ...
    return {
        status: 202,
        body: {
            generationId: job.id, status: 'queued', estCostUsd: estimate.usd,
