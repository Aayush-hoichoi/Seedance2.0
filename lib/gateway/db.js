...
export async function workflowStyle(sql, id, { userId = null, isAdmin = false } = {}) {
    ...
    const [row] = await sql`SELECT style FROM workflows w
        WHERE w.id = ${Number(id)} AND w.deleted_at IS NULL
          AND (w.created_by IS NULL OR w.created_by = ${userId} OR w.visibility = 'public' OR ${isAdmin})
...
// The workflows this user's picker shows: every official one (created_by
// NULL), every PUBLIC custom (owner asked + admin approved), and their own.
// Officials first, then name. creator_email attributes shared customs.
export async function listWorkflowsFor(sql, userId) {
    return sql`SELECT w.id, w.name, w.description, w.style, w.created_by, w.visibility,
            u.email AS creator_email
        FROM workflows w LEFT JOIN users u ON u.id = w.created_by
        WHERE w.deleted_at IS NULL
          AND (w.created_by IS NULL OR w.created_by = ${userId} OR w.visibility = 'public')
