...
export async function resolveOrgForUser(sql, sessionOrgId, userId = null) {
    if (sessionOrgId) {
        const [org] = await sql`SELECT * FROM organizations WHERE id = ${sessionOrgId} AND deleted_at IS NULL`;
        if (org) return org;
        // Session's active org isn't one we govern (e.g. an accidental
        // personal org Clerk spawned) — fall through to membership-based
        // resolution: the user's real workspace is where their projects are.
    }
    if (userId) {
        const orgs = await sql`SELECT DISTINCT o.* FROM organizations o
            JOIN projects p ON p.org_id = o.id AND p.archived_at IS NULL
            JOIN project_memberships m ON m.project_id = p.id AND m.user_id = ${userId}
            WHERE o.deleted_at IS NULL`;
        if (orgs.length === 1) return orgs[0];
        if (orgs.length > 1) return null; // genuine multi-org member with no valid active org
    }
    const orgs = await sql`SELECT * FROM organizations WHERE deleted_at IS NULL ORDER BY created_at ASC LIMIT 2`;
    if (orgs.length === 1) return orgs[0];
    return null; // none yet (migration pending) or ambiguous
}
