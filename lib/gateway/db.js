...
export async function workflowStyle(sql, id, { userId = null, isAdmin = false } = {}) {
    ...
    const [row] = await sql`SELECT style FROM workflows w
        WHERE w.id = ${Number(id)} AND w.deleted_at IS NULL
          AND (w.created_by IS NULL OR w.created_by = ${userId} OR ${isAdmin})
...
// NULL) plus their OWN customs — never another user's. Officials first.
export async function listWorkflowsFor(sql, userId) {
    return sql`SELECT id, name, description, style, created_by FROM workflows
