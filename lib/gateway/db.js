...
export async function reserveBillingEvent(sql, e) {
    ...
    const [, rows] = await sql.transaction([
        ...
        sql`WITH applicable AS (
                SELECT q.id, q.project_id, q.user_id, q.type, q."window",
                    COALESCE((
            ...
            SELECT 'reservation', ${e.generationId}, ${e.projectId}, ${e.userId}, ${e.modelId},
                   ...
                   ${e.pricingSnapshot == null ? null : JSON.stringify(e.pricingSnapshot)}
            WHERE NOT EXISTS (
                -- Every applicable scope is an independent cap. A user limit
                -- cannot borrow headroom from the shared project budget, and a
                -- personal allocation cannot bypass an exhausted project limit.
