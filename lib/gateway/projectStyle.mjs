...
const hasKeyword = (hay, words) => Array.isArray(words)
    && words.some((w) => typeof w === 'string' && w && hay.includes(w.toLowerCase()));

// Explicit look > keyword match > default. The keyword pass is what stops a
// yacht prompt from getting the car-chase brief: a look may declare `match`
// words, and the first look whose words appear in the prompt wins. Nothing
// matching falls back to the default, which is why default briefs must stay
// pure LOOK (see the scenes note below).
function looksResolved(style, requested, prompt = '') {
    const looks = style.looks && typeof style.looks === 'object' ? style.looks : {};
    const hay = prompt.toLowerCase();
    const matched = requested ? null : Object.keys(looks).find((k) => hasKeyword(hay, looks[k]?.match));
    const key = requested || matched || style.defaultLook || Object.keys(looks)[0];
    return key && looks[key] ? { key, look: looks[key] } : null;
}

// Scene/world locks are CONTENT, not look, so they follow the characters
// rule: injected only when the prompt actually references them (via each
// scene's `match` words). Baking a hero car or a market street into the brief
// itself made every unrelated prompt — "an elephant running in the jungle" —
// render the car chase: the block steamrolled the user's short prompt.
function matchedScenes(prompt, scenes) {
    if (!scenes || typeof scenes !== 'object') return [];
    const hay = prompt.toLowerCase();
    return Object.values(scenes)
        .filter((s) => s && typeof s.text === 'string' && s.text && hasKeyword(hay, s.match))
        .map((s) => s.text);
}
...
export function composeStyledPrompt(prompt, style, { look = null, maxPrompt = MAX_PROMPT } = {}) {
    ...
    const resolved = looksResolved(style, look, prompt);
    ...
    const scenes = matchedScenes(prompt, style.scenes);
    if (scenes.length) sections.push(scenes.join('\n'));
...
const STYLE_LIMITS = {
    looks: 12, brief: 4000, negatives: 2000, characters: 40, character: 1500, key: 64, name: 80,
    scenes: 20, scene: 2000, matchWords: 20, matchWord: 40,
};
...
export function styleError(style) {
    ...
    for (const key of keys) {
        ...
        if (look.match != null && matchError(look.match)) return `Look "${key}" ${matchError(look.match)}`;
    }
    ...
    if (scenes != null) {
        if (typeof scenes !== 'object' || Array.isArray(scenes)) return 'Scenes must be an object.';
        const keys2 = Object.keys(scenes);
        if (keys2.length > STYLE_LIMITS.scenes) return `At most ${STYLE_LIMITS.scenes} scenes per project.`;
        for (const key of keys2) {
            const scene = scenes[key];
            if (!scene || typeof scene !== 'object') return `Scene "${key}" is malformed.`;
            if (typeof scene.text !== 'string' || !scene.text.trim()) return `Scene "${key}" needs text.`;
            if (scene.text.length > STYLE_LIMITS.scene) return `Scene "${key}" text is too long (max ${STYLE_LIMITS.scene} characters).`;
            const err = matchError(scene.match);
            if (err) return `Scene "${key}" ${err}`;
        }
    }
    return null;
}
...
function matchError(match) {
    if (!Array.isArray(match) || !match.length) return 'match must be a non-empty list of keywords.';
    if (match.length > STYLE_LIMITS.matchWords) return `match may list at most ${STYLE_LIMITS.matchWords} keywords.`;
    if (match.some((w) => typeof w !== 'string' || !w.trim() || w.length > STYLE_LIMITS.matchWord)) {
        return `match keywords must be text up to ${STYLE_LIMITS.matchWord} characters.`;
    }
    return null;
}
