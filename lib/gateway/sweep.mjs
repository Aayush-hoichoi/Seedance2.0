...
import { isStalledClaim, maxAttemptsFor } from './queueLogic.mjs';
import { runLedgerTick } from '../ledger/sync.mjs';
import { drainLedger } from '../ledger/writer.mjs';
...
// The generation ledger, last and defensively. Every statement above this
// point affects whether generations run; nothing below it does. A spreadsheet
// being busy, a Graph outage or an expired Entra secret must never stop the
// queue from being processed, so both halves are caught and logged rather
// than propagated — and the Postgres half is attempted even when the Excel
// half is failing, because ledger_rows is the thing that has to stay correct.
async function syncLedger(sql) {
    try {
        await runLedgerTick(sql);
    } catch (err) {
        console.error(`[ledger] tick failed — ${err.message}`);
    }
    try {
        await drainLedger(sql);
    } catch (err) {
        console.error(`[ledger] drain failed — ${err.message}`);
    }
}
...
async function timeOutOverdueJobs(sql) {
    ...
    for (const job of overdue) {
        ...
        // a fresh invocation instead of failing, up to its attempt cap. processQueue
        // (run later in this same sweep) re-claims it immediately.
        if (isStalledClaim(job) && job.attempt < maxAttemptsFor(job)) {
