...
import { pickNextJob, backoffDelayMs, isRetryable, timeoutSecondsFor, maxAttemptsFor } from './queueLogic.mjs';
import { thresholdsCrossed, applicableQuotas, unitsForType, windowBounds } from './quota.mjs';
import { costFromTokens, budgetDuration } from '../seedance/pricing.mjs';
...
async function runWithFailover(sql, job, routing) {
    ...
    for (const route of routing.routes) {
        ...
        const r = await adapterFor(route.provider_id).submit({ job, route: routeCtx, apiKey: auth.key });
...
// Poll a freshly submitted async task (BytePlus video, kie image) inside this
// invocation's budget; the sweep resumes polling on later traffic if the task
// outlives it.
async function pollUntilBudget(sql, job, routing, auth) {
    const deadline = Date.now() + POLL_BUDGET_MS;
    const adapter = adapterFor(job.provider_id);
    while (Date.now() < deadline) {
        await new Promise((r) => setTimeout(r, 5_000));
        const p = await adapter.poll({ job, apiKey: auth.key });
        if (p.ok && p.done) {
...
// Re-poll everything already running (provider tasks + google batches).
export async function pollRunningJobs() {
    ...
    for (const job of running) {
        ...
        if (!auth) continue;
        const p = await adapterFor(job.provider_id).poll({ job, apiKey: auth.key });
        if (p.ok && p.done) {
...
async function settleSuccess(sql, job, { route, apiKeyId, result, usage, kind, mode }) {
    ...
    if (!images && usage?.completion_tokens != null) { // video only: images never take the token-cost branch
        ...
        units = { video_seconds: budgetDuration(kind, req.duration), completion_tokens: usage.completion_tokens };
        snapshot = { basis: 'tokens', kind, resolution: req.resolution };
    } else if (images) {
        const routeMode = mode || route.mode || 'interactive';
        costUsd = imageCost(kind, routeMode, images.length, req.imageSize, req.quality);
        units = { images: images.length };
        snapshot = { basis: 'per_image', kind, mode: routeMode, imageSize: req.imageSize ?? null, quality: req.quality ?? null, rate: imageRate(kind, routeMode, req.imageSize, req.quality) };
    }
...
async function retryOrFail(sql, job, error) {
    if (job.attempt < maxAttemptsFor(job) && isRetryable(error)) {
