...
// The deterministic floor for a custom workflow: the user's own words become
// the brief, untouched. Always passes styleError (brief is length-capped), so
// creation NEVER fails on the model being down or returning junk.
export function fallbackStyle(name, description) {
    return {
        enabled: true,
        version: 1,
        defaultLook: 'main',
        looks: { main: { name: name.slice(0, 80), brief: description.slice(0, 4000) } },
    };
}

// Turn a plain-words style description into a structured style bible via the
// enhancer model — same schema, same styleError guardrails as the nightly
// refresh, same hard rule that briefs stay pure LOOK (the Good Samaritan
// content-bleed lesson). Falls back to the user's raw text as the brief.
export async function draftWorkflowStyle({ name, description, examplePrompt = null }) {
    try {
        const apiKey = process.env.OPENAI_API_KEY?.trim();
        if (!apiKey) return fallbackStyle(name, description);
        const model = process.env.OPENAI_ENHANCE_MODEL?.trim() || 'gpt-5.6-luna';
        const res = await fetch(OPENAI_URL, {
            method: 'POST',
            headers: { Authorization: `Bearer ${apiKey}`, 'Content-Type': 'application/json' },
                ...
                messages: [
                    {
                        role: 'system',
                        content: [
                            'You write the style bible for an AI video/image generation workflow from a user\'s plain-language description. Return ONLY a JSON object with this exact schema:',
                            '{ "enabled": true, "version": 1, "defaultLook": "<key>", "looks": { "<key>": { "name": string, "brief": string, "negatives"?: string, "match"?: string[] } }, "characters"?: { "<Name>": string }, "scenes"?: { "<key>": { "match": string[], "text": string } } }',
                            'Rules:',
                            '- A brief is PURE LOOK: rendering style, lighting, palette, camera and motion rules, performance notes. It is appended to every prompt.',
                            '- Recurring subjects/people go in `characters` (injected only when a prompt names them). Locations, vehicles and world descriptions go in `scenes` with `match` keywords (injected only when a prompt mentions them). NEVER put subjects or places in a brief.',
                            '- One look unless the description clearly names distinct looks. Give non-default looks `match` keywords.',
                            '- Write negatives from what the description implies must never happen (style drift, wrong medium).',
                            '- Limits: brief ≤ 4000 chars, negatives ≤ 2000, each character ≤ 1500, each scene text ≤ 2000, match ≤ 20 words.',
                        ].join('\n'),
                    },
                    {
                        role: 'user',
                        content: `Workflow name: ${name}\n\nStyle description:\n${description}${examplePrompt ? `\n\nAn example prompt that nails the intended look:\n${examplePrompt}` : ''}`,
                    },
                ],
            }),
        });
        if (!res.ok) return fallbackStyle(name, description);
        const data = await res.json();
        const proposed = JSON.parse(data?.choices?.[0]?.message?.content ?? 'null');
        const style = { ...proposed, enabled: true, version: 1 };
        return styleError(style) ? fallbackStyle(name, description) : style;
    } catch {
        return fallbackStyle(name, description);
    }
}
