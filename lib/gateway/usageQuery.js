...
const GROUP_COLS = {
    ...
    // IST day buckets, matching the ledger's "Date (IST)" — a UTC bucket made
    // "Spend today" reset at 05:30 IST and disagree with the ledger by
    // whatever was spent between midnight and 05:30.
    day: `to_char(b.created_at AT TIME ZONE 'Asia/Kolkata', 'YYYY-MM-DD')`,
};

export async function usageRollup(sql, { projectId = null, groupBy = 'model', from = null, to = null }) {
    // day_user / day_model / day_project: one row per (day, series) so charts
    // can draw a spend line per user, model or project. user_model is the same
    // two-dimensional shape for detailed hover breakdowns on per-user bars.
    const daySeries = ['day_user', 'day_model', 'day_project'].includes(groupBy) ? groupBy.slice(4) : null;
    const isUserModel = groupBy === 'user_model';
    const hasSeries = Boolean(daySeries) || isUserModel;
    const col = daySeries
        ? GROUP_COLS.day
        : isUserModel
            ? GROUP_COLS.user
            : (GROUP_COLS[groupBy] || GROUP_COLS.model);
    const seriesCol = daySeries === 'user' ? GROUP_COLS.user
        : daySeries === 'project' ? GROUP_COLS.project
            : 'COALESCE(m.display_name, b.model_id)';
    const joinModels = isUserModel || daySeries === 'model';
    const joinProjects = groupBy === 'project' || daySeries === 'project';
    const rows = await sql.query(
        `SELECT ${col} AS key,${hasSeries ? ` ${seriesCol} AS series,` : ''}
         ...
         ${joinModels ? 'LEFT JOIN models m ON m.id = b.model_id' : ''}
         ${joinProjects ? 'LEFT JOIN projects p ON p.id = b.project_id' : ''}
