...
// Seedream 5.0 Pro accepts at most 10 reference images. sanitizeImageRequest
// already caps by model, so this is a defence-in-depth ceiling for anything
// reaching the adapter another way — over the limit the API rejects the whole
// request, losing the generation rather than a reference.
const SEEDREAM_MAX_REFS = 10;

// {inlineData:{mimeType,data}} (Gemini shape, how the studio stores refs) →
// "data:<mime>;base64,<data>" (what images/generations wants in `image`).
// Text parts carry the prompt, which is already sent as `prompt`, so they are
// dropped here. A malformed entry is skipped rather than failing the job: the
// validator has already vetted these, and losing one reference beats losing
// the whole generation.
export function seedreamRefs(parts) {
    if (!Array.isArray(parts)) return [];
    return parts
        .filter((p) => typeof p?.inlineData?.data === 'string' && p.inlineData.data)
        .slice(0, SEEDREAM_MAX_REFS)
        .map((p) => {
            const data = p.inlineData.data;
            // Tolerate an already-complete data: URI — double-prefixing yields
            // "invalid url specified" from the API.
...
// Bound every ModelArk call just under the function's maxDuration (300s) — the
// sync Seedream image call can be slow, and an unbounded fetch strands the job
// (no retryable signal, no handle to resume) until the sweep times it out.
const ARK_TIMEOUT_MS = Number(process.env.BYTEPLUS_TIMEOUT_MS) || 290_000;

async function arkFetch(path, { method = 'GET', apiKey, body } = {}) {
    const ctl = new AbortController();
    const timer = setTimeout(() => ctl.abort(), ARK_TIMEOUT_MS);
    try {
        ...
        if (e.name === 'AbortError') return { ok: false, error: { code: 'ETIMEDOUT', message: `The provider did not respond within ${Math.round(ARK_TIMEOUT_MS / 1000)}s.` } };
...
export async function submit({ job, route, apiKey }) {
    ...
    if (rb.category === 'image' || route.category === 'image') {
        ...
        // References. The studio stores them Gemini-shaped ({inlineData}), and
        // stripping the whole `parts` array used to drop them silently: every
        // Seedream job carried refs, none of them ever reached the model, and a
        // prompt like "change the lamp in Image 1 to the one from Image 2" was
        // answered from the text alone. Seedream takes them as `image` — an
        // array of URLs and/or data: URIs, verified against the live API.
        const refs = seedreamRefs(rb.parts);
