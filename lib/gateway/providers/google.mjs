...
async function glFetch(path, { method = 'GET', apiKey, body } = {}) {
    ...
    try {
        ...
        // undici throws a bare `TypeError: fetch failed` and hides the real
        // fault (socket reset, headers timeout, DNS) in e.cause. Dropping the
        // cause stored every transport failure as the same useless string —
        // unroutable by friendlyError and undiagnosable after the fact.
        const cause = e.cause || {};
        return { ok: false, error: {
            code: cause.code || e.code || 'ENETWORK',
            message: [e.message, cause.message].filter(Boolean).join(': '),
        } };
    } finally {
        clearTimeout(timer);
    }
