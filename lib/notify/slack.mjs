...
export function buildAccessRequestedMessage({ id, email, modelId, projectName, note }) {
    ...
    if (id != null && process.env.SLACK_SIGNING_SECRET) {
        // A datepicker for a custom expiry; its picked date rides in the button
        // click's state.values, read by the handler when "Use picked date" fires.
        blocks.push({
            type: 'actions', block_id: 'access_expiry',
            elements: [{
        ...
        // Preset windows + a "use the picked date" option; value carries id + window.
        const buttons = APPROVE_PRESET_DAYS.map((days) => ({
            type: 'button', style: 'primary', action_id: 'access_approve', value: `${id}:${days}`,
            text: { type: 'plain_text', text: `✅ ${days} days`, emoji: true },
            confirm: approveConfirm(model, email, `${days} days`),
        }));
        buttons.push({
            type: 'button', style: 'primary', action_id: 'access_approve', value: `${id}:custom`,
        ...
        if (base) buttons.push({ type: 'button', text: { type: 'plain_text', text: 'Review in console' }, url: `${base}/console/users` });
