...
// A member asked for a NEW project. Approve creates it and adds them to it —
// the confirm dialog says exactly that so a tap can't create by accident.
export function buildProjectRequestedMessage({ id, email, name, note }) {
    const base = appBase();
    const fields = [
        { title: 'User', value: email },
    ...
    const blocks = [
        { type: 'header', text: { type: 'plain_text', text: '📁 New project request', emoji: true } },
        { type: 'section', fields },
    ];
    if (id != null && process.env.SLACK_SIGNING_SECRET) {
        const elements = [
            {
                type: 'button', style: 'primary', action_id: 'project_approve', value: String(id),
                text: { type: 'plain_text', text: '✅ Create & add', emoji: true },
                confirm: dialog('Create project?', `Create *${esc(name)}* and add ${esc(email)} to it?`),
            },
            {
                type: 'button', style: 'danger', action_id: 'project_deny', value: String(id),
                text: { type: 'plain_text', text: '🚫 Deny', emoji: true },
                confirm: dialog('Decline request?', `Decline the project *${esc(name)}* for ${esc(email)}? Nothing is created.`),
            },
        ];
        if (base) elements.push({ type: 'button', text: { type: 'plain_text', text: 'Review in console' }, url: `${base}/console/projects` });
        blocks.push({ type: 'actions', block_id: 'project_decision', elements });
    } else if (base) {
        blocks.push({ type: 'actions', elements: [{ type: 'button', text: { type: 'plain_text', text: 'Review in console' }, url: `${base}/console/projects` }] });
    }
    return { text: `${mention()}New project request: ${email} → ${name}`, blocks };
}

export function buildProjectDecidedMessage({ email, name, status }) {
    const approved = status === 'approved';
    return slackMessage(
        `${approved ? 'Created' : 'Declined'}: project ${name} for ${email}`,
        approved ? '📁 Project created' : '🚫 Project request declined',
        [
            { title: 'Project', value: name },
            { title: 'User', value: email },
            { title: 'Status', value: approved ? `Created — ${email} added` : 'Not created' },
        ],
    );
}
...
export function notifySlackProjectRequested(args) {
    return post(buildProjectRequestedMessage(args));
}

export function notifySlackProjectDecided(args) {
    return post(buildProjectDecidedMessage(args));
}
