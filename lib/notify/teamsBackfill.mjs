...
export async function backfillTeamsCards({
    ...
    // [teams] logs immediately above (usually an id in TEAMS_ADMIN_AAD_IDS that
    // no longer has the bot installed, or is not a user in this tenant).
    ...
    if (undelivered) {
        console.error(`[teams] backfill could NOT deliver ${undelivered} card(s) — `
            + 'check that every id in TEAMS_ADMIN_AAD_IDS is a tenant user with the bot installed');
    }
