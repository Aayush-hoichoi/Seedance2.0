...
import {
    appBase, approverEmails, teamsConfigured, botToken, resolveAadObjectId,
...
export async function notifyTeamsAccessRequested({ requestId, request, sql: providedSql = null }) {
    ...
    try {
        ...
        const results = await Promise.allSettled(approverEmails().map(async (email) => {
            ...
            const approveUrl = `${base}/api/webhooks/teams-access-approve?token=${encodeURIComponent(
                signApprovalToken({ kind: 'access', requestId, adminUserId: admin.id, aadObjectId, action: 'approve' }),
            )}`;
            const denyUrl = `${base}/api/webhooks/teams-access-approve?token=${encodeURIComponent(
                signApprovalToken({ kind: 'access', requestId, adminUserId: admin.id, aadObjectId, action: denyAction }),
            )}`;
            const card = buildAccessRequestCard(request, requestId, { approveUrl, denyUrl });
